...
- **取样**：`unwrapped._reset_idx` 临时包装，在复位发生前抓 `head_contact_max(...)`（瞬时与 **传感器自身
  ...
  `terminated`/`truncated`。
- **对照（同一台仪器）**：站立不动 40 步 = 头部接触力峰 **0.00 N**、守卫不触发；压头下降 = 守卫触发、
  触发帧力 **1080.29 N**、同帧窗口最大 **1080.29 N**，且该帧头部网格在地面**下 724 mm**（几何能解释力），
  返回 `terminated=True`、`truncated=False`、`base_contact` 不在其中。
...
- **不是阈值标定**：机器人站在脚上时头部够不到地面（极限头是把鼻子朝下
  的机体**放进**地面，故读到的是求解器对该放置的响应。它证明的是"声明的连杆承力时守卫会触发、不承力时安静"，
  **不是**"1 N 阈值在 1 N 处翻转"；从下方穿过阈值的温和下压仍欠着。
- 几何读数用的是资产自己的碰撞网格（`physics:approximation = "convexHull"` 的凸包近似），与运行时碰撞体
  可差到厘米级；因此 `head/fires-with-the-head-at-the-floor` 的容差按一次下压步长取，不做更细的深度断言。
- 该命令判据的期望现在**要求 yaml 点名生效集合**：未写 `terminations.terms` 的旧线跑本探针会红
