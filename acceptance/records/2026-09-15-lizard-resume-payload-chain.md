# Lizard 续训与恢复：运行记录 / 载荷 / 隔离重建（2026-09-15 五批 + Step 1 汇总复核）

## 适用范围

本记录搬运 `rl_exp/versions/lizard/ACCEPTANCE.md` 的以下旧节（按 2026-09-15 的追加序）：

| 旧节 | 主题 |
|---|---|
| §1.2b | 运行记录运行时验收（R1–R10）+ 期间的 golden 重基线与路线级锁拆分 |
| §1.3a | 载荷与接口离线验收（S01–S10 + A/B 层） |
| §1.5a | 恢复演练工具与一次完整演练（P1–P13） |
| §1.4a | 恢复验收 B 层补全（分支级 + c_k 迭代边界） |
| §1.4b | 恢复验收 C 层（真跑，三条代表线 × resume/drop） |
| §Step 1 汇总复核 | 把 1.3a/1.5a 前提里「提交后须按同表重跑」闭合到 revision |

**本记录内部的前后作废关系（读结论以此为准，被作废的旧句保留作留痕）**：

- §1.2b「本批未覆盖」里 **1.5 隔离重建** 与 **1.4b C 层代表任务** 两条，已被同批晚些的 §1.5a 与
  §1.4b 判定**已执行**；上述两条的「未执行 / 恒定未知」结论自此失效，以本记录 §1.5a / §1.4b 为准。
  第三条（1.3a S01–S10）由本记录 §1.3a 承接。
- §1.2b「已知瑕疵」前两条（fork 补丁仍以 `weights_only=...` 调用、`REQUIRES_CURRICULUM_STATE`
  基类声明缺失）已修，原句以删除线保留。
- **配方摘要的可比性随快照格式变化**（本节自带口径，且被 §Step 1 复核继承）：格式 **1** 记录的配方项
  **一律降级为未知**（不可比），不据旧摘要判失败或通过；格式 **2** 起可比（R1′）；**2 → 3**
  一个配置值都没动，故格式 2 的配方项**不**降级。
- **R4 的「代码可取回」仍是未知**（归档绑的是 19-08-50 那次 run，不是 R1 的 `cc25f24b`）。
- §1.5a 的结论只在**本次演练范围**内成立（重建项目材料 + 复用声明的现有环境），不外推环境重装。

**读本记录须知的通读口径**（与 `ACCEPTANCE.md` 抬头一致）：历史记录里的 `[N]` 是**当次运行编号**，
不是闸门身份；套件总条目数受 `MAX_CHECKS` 棘轮管 ⇒ 各节的成功行读数**只在当日 commit 上成立**；
「已修」与「仍是缺口」并存时，作废句一律从作废它的那节读。

不进本记录：`社会控制` / `静默跳过` / `不可复跑` / `首回合失败` 等未闭合项各有其主（见 `ACCEPTANCE.md` 路由表）。

## 验收条件

通则（`ARCH_PLAN.md` v0.12「验收记录通则」）：每次验收记检查编号、任务 id、命令、代码/框架与资产摘要、
设备与 env 数、seed、输入 checkpoint/样本摘要、预设容差、实际结果、证据路径与通过/失败/未知。
**验收标准是实施要求，不等于已通过**：未跑、前提不满足、证据缺失一律记「未知」。
...
| 证据路径 | 每臂 `%TEMP%\c4\<arm>\observe_<pid>.json` + `<arm>.log`；汇总 `%TEMP%\c4\c_layer_report.json`；run 目录见本记录「臂与 run」小节 |
| 负对照 | `%TEMP%\falsify_clock_rule.py`：把时钟规则改回 `next_eval <= counter` ⇒ 新用例失败；还原后通过（**FIRED**） |

### §Step 1 汇总复核前提（本次复核绑定的对象）
...
**本批出口判定（2026-09-15 晚，F3 补记）**：本批**必需**验收 ①②③④ **通过**（R2 动态模型类进 T1、
R3 恢复后生效 lr、R5/R6 失败无 T1、R7/R8 引用漂移与未声明来源判失败）；**R4 的两项未知属非必需项**
（未跟踪代码可取得、格式 1 配方摘要可比性），**不阻塞本阶段出口**，原判定与边界一并保留。
本节追加说明只补「材料在哪、摘要多少」，**不把当时的未知改判为通过**。
...
- **1.5 隔离重建**：无重建记录，`已验证重建` 恒为**未知**（非必需项，不影响其它结论）。
  - **追加（2026-09-15 晚）**：已执行一轮按需演练（重建项目材料 + 复用声明环境），范围内通过，见本记录 §1.5a 节；本批结论不受影响。
- **1.4b C 层代表任务**（c_k-only `v3`、行 SIR `v14`、joint SIR `v12` 的真续训）：**未执行**。
  - **追加（2026-09-15 晚）**：**已执行**（3 代表任务 × resume/drop，15 臂；`afd1e54`），结论见本记录 §1.4b 节。
- **1.3a S01–S10**（载荷/指纹/损坏/边界/兼容）：**已执行（离线）**，见本记录 §1.3a 节。
...
- **不可**据本表宣告：运行时续训连续（C 层见本记录 §1.4b）、多卡续训已验证（明确不在保证范围）、
  drop 路径的 rsl_rl 加载行为（未真跑，后由 §1.4b C7 补上）、隔离重建（见本记录 §1.5a）。
...
- 可**宣告**：**1.4a A/B 层离线验收通过**（A 层 S01–S10 见本记录 §1.3a）。
- **不可**据本节宣告：接线时序正确（apply 早于 wrapper 首次 reset）——只有 C 层真跑能证（见本记录 §1.4b）；
  冷热前 N 步逐位一致（RNG/出生/per-episode 瞬态刻意不存）；多卡续训（不在保证范围）。
...
**本批修正：`_check_eval_clock` 曾把合法 ckpt 判为不可恢复**（真跑抓到，A/B 层看不见）：
...
### §Step 1 汇总复核：结果与框架漂移（286 → 344 行）影响审查

**性质**：**追加**条目。历史各节的原摘要与原结论**不改**（硬约束 6）；此前 1.3a / 1.5a 前提里"提交后须按同表重跑"的要求由此闭合到 revision（前提表见本记录 §验收条件）。
...
- 运行记录本体：各 run 目录 `logs/rsl_rl/lizard_rough_teacher_v14/<时间戳>`（R1 `15-30-08`、R1′ `15-58-41`、
  R3 恢复 run `15-31-28`、R5 `15-33-25`、R6 `15-33-58`、R10 修复前记录 `15-24-34`；R9 复核仍指 R1）；
  单跑命令 `python -m rl_exp.tools.runrecord.manifest --verify <run_dir>`。
- 注入式反证脚本：`%TEMP%\rm_2b_inject.py`、`%TEMP%\falsify_guards.py`、`%TEMP%\falsify_b_layer.py`、
  `%TEMP%\falsify_clock_rule.py`；注入产物 `%TEMP%\rm_inject_asset_drift`、`%TEMP%\rm_inject_undeclared_source`。
- C 层：每臂 `%TEMP%\c4\<arm>\observe_<pid>.json` + `<arm>.log`；汇总 `%TEMP%\c4\c_layer_report.json`；
  判据入口 `python rl_exp\tools\verify\check_c_layer.py --root %TEMP%\c4 --resave --source-checkpoint v14=<ckpt> …`
  → `C_LAYER_OK (0 failing)`。
- 演练：`%TEMP%\rebuild_r1_dryrun\rebuild_material.json`；材料与脏树归档在仓内 `rl_exp\archive\2026-09-15_19-08-50\`
  （含 `isaaclab.diff` / `rsl_rl.diff`）；取材 run
  `logs\rsl_rl\lizard_rough_teacher_v14\2026-09-15_19-08-50`（T1 `aeba04549bc363f1`）。
- 离线套件与日志：`rl_exp\tools\verify\run_offline_checks.bat`；Step 1 日志
  `rl_exp/versions/lizard/verify_logs/step1-offline-00d0b07-2026-09-15.log`（入库）。
- golden 与锁：`rl_exp/versions/lizard/cfg_lock.json`（格式 3 前为全量单文件）、
  `rl_exp/versions/lizard/parkour/cfg_lock.json`、`rl_exp/versions/cfg_baselines.json`；
  格式 1 旧基线取回 `git show 7525268:rl_exp/versions/lizard/cfg_lock.json > <out>`。
- 契约与改动面：`rl_exp/tasks/curriculum_state.py`、`rl_exp/tools/runrecord/manifest.py`、
  `rl_exp/tools/verify/{test_resume_state.py,test_run_manifest.py,cstate_observer.py,check_c_layer.py,
  rebuild.py,test_rebuild_gate.py}`、`rl_exp/fork_patches/*.patch`。

## 未覆盖边界

- **不可据本记录宣告的（逐节边界，全部保留）**：
  - §1.3a：运行时续训连续（当时待 §1.4b）、多卡续训、drop 路径的 rsl_rl 加载行为（当时未真跑）、隔离重建。
  - §1.3a S07：**调用侧**（含模块内部 ImportError）只以补丁静态契约 + 树一致性校验覆盖，**未起真进程**。
  - §1.5a：**不可外推** —— 环境未重装 ⇒ 不证明环境安装过程可复现；未做系统级访问切断 ⇒ 不覆盖隐藏读取
    （包内部 / 缓存 / 环境变量旁路）；2 iter run ⇒ 不承诺推理或训练结果等价。结论只在"本次演练范围"内成立。
  - §1.4a：不证接线时序（属 C 层）；不证冷热前 N 步逐位一致；不证多卡。B 层证明的是"状态 → 更新映射"一致，**不是复现**。
  - §1.4b：不证"复现"；不覆盖 v5–v10、v13 的逐版本真跑；多卡续训不在保证范围；84 it 不证长期训练统计等价。
  - §Step 1：框架漂移的**逐文件归属不可证**（不得反推"只差 train.py"）。
- **未闭合项（仍开着，不得据本记录判已解决）**：
  - **R4「代码可取回」仍为未知**；升级为通过的条件写在 §1.2b「已知瑕疵」最后一条
    （T1 需带未跟踪文件的逐文件内容摘要）。
  - **无归档的"可取回"项一律未知**（`PLAN.md` #18）。
  - 声明仍会出现在 `to_dict()` / `params/env.yaml`（记录面）；要连它一起干净需动上游 `utils/dict.py`，**本轮不做**。
  - `cfg_snapshot.py` 曾被两方并发写入 → **后续应避免同文件并发编辑**（未加固，只是纪律）。
  - §Step 1 新登记的**风险**未修：fork 树里的 DR 放大（模式与节拍走继承）会静默改变新 run 的 DR，
    只有 golden 差异能抓到；建议把三处 DR 改动显式收编进本仓参数。
  - **社会控制**（「生成 + 人工审批」无痕迹）、**静默跳过**、**不可复跑**、**首回合失败** 等跨主题未闭合项
    不在本记录，见 `ACCEPTANCE.md` 路由表指向的记录。
- **口径边界**：历史 `[N]` 是当次运行编号而非闸门身份，各节成功行读数只在**当日 commit** 上成立；
  「已修」与「仍是缺口」并存时以作废它的那节为准。
