...
| `tools\verify\run_offline_checks.bat` | **离线全套一键**（30 项：pin/parity/recovery/staged 课程/teacher 网络/student 网络/v3 课程+环形/obs 布局/v5 奖励/v5 SIR/v11 联合 SIR/版本文档/v12 噪声模型/pxr 泄漏闸/续训状态/tb 抽样/v13 跟踪核/v14 摔倒闸/验收量测（同帧+abs 侧滑）/eval 帧契约 v2/configclass 字段面 + 其负测试/配置序列化器/**配方 golden 锁 + 其负测试/运行记录/配方线发现 + 其负测试/生命周期闸门 + 其负测试**，秒级不起仿真）；改 tasks 或 harness 后、commit 前必跑 |
