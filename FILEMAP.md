...
| `recipe.py` | **配方的声明式构建**（`ARCH_PLAN.md` §2.4 阶段 B3 起步）：`RECIPES` 给出每个配方的**有序元素表** + 它声称复现的注册任务 id；`base_cfg(version)` = 元素之前的共享接线（`LizardRoughTeacherEnvCfg(params_version=…)`，版本作为**字段**传入 ⇒ 不经版本类），`build(version, play=, trace=)` = 共享接线 + 该配方的元素，`trace` 按序记下每一步（`(name, callable)`）让读者**回放**而不是复述顺序；`pending()` 列出**尚未声明**的版本（未声明 ≠ 通过）；`declares` 是**每条配方自己的承诺**（`(train, play)`，真源在这里，`build()` 以 ClassVar 盖在合成子类上）。硬 A 由 `tools\verify\check_recipe_build.py`（套件 `[41]`）以冻结 golden 比对，另有三条收尾判据：**覆盖钉数**（`EXPECTED_COMPARED`，比较集静默缩小即红）、**缺口台账**（`EXPECTED_GAPS` 按 ClassVar 名给理由与到期，台账外的新缺口即红，台账里的陈旧项也红）、**逐步归属**（每个声明元素必须改到字段，且全部差异必须有人认领 —— 空转元素在字段比对上隐形，只有逐步求差能抓）；该闸门另**打印**声明路径结构上带不走的 `ClassVar`（快照格式 2 排除 `ClassVar`，而 `build()` 只返回共享基类 ⇒ 没有类可承载它；`REQUIRES_CURRICULUM_STATE` 是运行期读取，属真缺口，到期条件见台账） |
...
| `lizard\main\v6\` | **已训判废**（2026-09-08 停训 8950/15000：**倒走**——rig 骨命名与解剖学 180° 装反，"+Y→+X"转的是命名头=解剖学尾；探针/归因见 `v6\NOTES.md`）：资产前向轴转正（blend R_z(-90°)、AXIS_MAP 轴同步、锁全版本刷新）+ v6.1 脊柱/尾动作解锁（`joint_pos_spine` 0.0→0.25）；yaml/reward/obs 逐字同 v5，动作 26 维布局不变——关节专项（`check_joints_v6.py`/`check_skeleton_equivalence.py`）与装配验证记录见 `v6\NOTES.md` |
| `lizard\main\v8\` | **提案待训**（2026-09-08，v6 判废根因根治）：资产**解剖学**转正（blend 再转 R_z(+180°)，对 pre-v6 净 +90°，**球头**→+X、天线尾→−X）+ 全关节重命名（rear→chest、tail→neck（球头颈）、neck1-3→tail1-3（天线尾）、腿 rl↔rf/lf↔rr 换正）+ 全版本 yaml 机械迁移；reward/obs/动作布局逐字同 v6.2。布局硬闸（球头必 +X）入 `check_joints_v8.py`。SSOT = `v8\NOTES.md` |
...
| `tools\runrecord\lifecycle.py` | **启动生命周期判定（ARCH_PLAN 2.2 / C1）**：`read_index()` 读一次 `lines.json`+`recipes.json`（带 revision 与逐文件 sha256）；`identity()` 把 task id 解析为「配方键 → 线 → 状态」，任一步缺失即**拒绝**（未登记任务 / 线无生命周期条目 / 索引读不了，都不许读作 active）；`retirement_due()` 复用离线闸门的 `condition_satisfied` 判公告是否到期（运行期只 WARN 并记录）；`source_checkpoint()` 用框架自己的 `get_checkpoint_path` 解析 resume 源（**在 T0 建目录之前**解析：否则默认 `load_run=".*"` 会匹配到本 run 的空目录）；`startup_check()` 调冻结契约 `recipe_lifecycle.judge()` 并产出 T0 记录的证据。`RL_RECIPE_DIR` 可把"读哪份目录"指到 fixture（2.1a 禁止真退休一条线） |
| `tools\verify\lifecycle_entry_fixture.py` | 上者的 fixture 生成器：复制两份索引并把一条线翻成 `retired`（或给它一个到期公告），写 `FIXTURE.md` 并打印运行命令。**不进离线套件**（要起 sim 才到得了拒绝），是 C4 真跑窗口的输入 |
| `tools\verify\lifecycle_entry_run.py` | **入口侧真跑（C4 / L02、L03、L05）**：五档 = `launcher`（新启动器在 spawn 前拒，不需 sim）/ `trainer`（旧 trainer 拒绝**且 T0 留下拒绝记录**）/ `tuning`（`run_ablation` 走同一子进程，必须被杀死）/ `announce`（到期公告 WARN 并放行 + 记录）/ `moved`（启动后目录改版 ⇒ `verify` 只记注不重判、记录不被改写）。逐档断言命令、退出码、输出与 T0 字段；`--dry-run` 列出哪些档要起 sim |
| `tools\launch_recipe.py` | **新启动器（C2）**：目录 → 身份（复用 `lifecycle.read_index/identity`）→ 配置侧绑定（复用 `check_recipe_map.bind`）→ 按声明入口构建 → golden 对比（复用 `manifest.recipe_ref`）→ **同一个** `lifecycle.startup_check` → 记录（拒绝以 `allowed=false` 返回而非抛错，便于归档）。默认只检查（不起 sim）；`--launch` 交回 fork trainer 并把 `RL_RECIPE_DIR` 钉成"本次判定所用的同一份目录"（T0 因此绑同一份）。**待切**：`plan()` 仍构造声明入口的类，等注册表切到 `recipe.recipe_class()`（[41] 已断言两者逐字段一致） |
| `tools\verify\test_lifecycle_gate.py` | 上者的离线闸门（套件 `[39]`）：22 组用例 + 反证 4 例（含"身份分歧静默通过"）；含目录重定位与"拒绝必须留 T0"两组 |
