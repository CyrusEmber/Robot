...
| `lizard\cfg_lock.json` | **配方 golden（家族级，闸门产物）**：`baselines[框架组合] → entries[任务 id]`（34 个任务，env+agent 解析后字段树 + 摘要 + `params_version`）；**跑 run 不增长**，只在框架组合（isaaclab/rsl_rl/python）变化时新增独立 block。不写进 `vN\`（冻结目录只读，`check_version_docs.py` 看守 vN 五件套）。`created_dirty` 为真表示该基线不干净（提交后重生成）；改配置而未 `--update --reason` 重生成即红 |
...
| `tools\verify\test_resume_state.py` | 真续训状态层离线单测（mock env，复用 `test_joint_sir`/`test_v5_terrain_sir` 夹具，26 项 = A 层 S01–S10 + B 层）：逐位往返 + **c_k 连续**（不回热）/ 静态指纹抓"地形网格改动而 n_pairs 不变"的静默错位 / 速度桶与任务身份不符拒恢复 / 评估时钟与权重一致性拒恢复 / 无 joint SIR 任务透传 / 有 term 无状态**硬中断** + `--drop_curriculum_state` 显式降级（旧 `--weights_only` 别名）/ save hook 注入 + 参数透传 + `wraps` 保名 / `env_type` 漂移告警并还原 / 未覆盖课程 term 绊线 / 行 SIR 与 joint SIR 的**分支级 B 层**（测量更新、`n_traj_min` 以下、全带外回退、游走、replay：先断言分支签名，再走生产入口 `term(env, ids)`，原对象与恢复对象逐位一致）/ c_k-only 的迭代边界比对（按载荷保存参数独立重算 float64，误差 ≤1e-12）。**注**：只证载荷往返与"状态→更新映射"，接线时序（恢复早于首次 reset）只能真跑证（1.4b C 层） |
...
| `tools\runrecord\rebuild.py` | **隔离重建（ARCH_PLAN 1.5，离线半）**：`--capture <run_dir> <dest>` 按 T1 声明取材（干净代码按 rev 可取回、脏树 diff 与未跟踪代码**必须**有 `--archive` 落点，否则**拒采且不写材料**，只留拒采记录 —— `PLAN.md` #18 变成闸门）+ ckpt 与 run 记录拷贝并逐文件摘要；`--check <dest> --root <重建根>` 查四问：材料摘要仍对、导入模块与资产锁落在重建根（`--dep` 声明依赖例外）且**不落回 `--original` 给的原源码/资产树**、ckpt `infos` 回指本记录 T1、原目录可读性只作信息行（v0.16：不做系统级访问切断）；`--maintest` 删一个必需载荷必须失败、还原必须通过。不起仿真，只到"受检查路径上的配置与加载级" |
| `tools\verify\test_rebuild_gate.py` | 上者离线验证（28 项，复用 `test_run_manifest._record` 造 run 记录）：漂移脏树拒采、无归档拒采、带归档落盘且摘要一致、未跟踪代码内容缺失拒采、材料完好/来源/载荷绑定三行、解析落回原树或未声明位置判失败、未给 `--original` 记未知、原目录可读只作信息、删载荷必失败+还原必通过、被改载荷与冻结后被改记录均不得通过、拒采记未知（退 2）而失败记阻塞（退 1） |
