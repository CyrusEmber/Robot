...
| `lizard_env_cfg.py` | **家族接线根** + `_load_params`：`LizardFlatEnvCfg`（机器人装配 + DR 接线 + 平地基座）与 `_PLAY` 是四个家族开发态配方共同声明的 `base`（配方表里的 `"base": lizard_env_cfg.LizardFlatEnvCfg`）。**2026-09-18 起它同时是声明路径的根**：`flat-v0` 的整个内容**就是**本类（元素为空，同 teacher 线 v1 的形状），`rough-v0`/`curriculum-*-v0` 以元素在其上做差 |
| `rough_env_cfg.py` | 只留 `LIZARD_ROUGH_TERRAINS_CFG`（蜥蜴尺度化地形生成器常量，元素 `v0_rough_terrain` 深拷贝它）。**类体已删（2026-09-18）**：`LizardRoughEnvCfg`/`_PLAY` 的内容由 `rough-v0` 的两个元素（地形 + 高度扫描）声明；地形**调参点仍是这个常量**，元素不重述阈值 |
| `curriculum_env_cfg.py` | 只留 `LizardCurriculumActionsCfg`（动作拆分）+ `_make_stages(spine_scale)`（三课程 stage 表 = 调参点）。**类体已删（2026-09-18）**：`curriculum-flat-v0` / `curriculum-rough-v0` 用元素组合前者、后者与 rough delta |
...
| `terrain_map.py` | **唯一的列切分规则 + 真实生成记录的形状**（`ARCH_PLAN.md` Step 3.3b/3.3c，纯 stdlib 不 import isaaclab）：`column_split` 复刻 `TerrainGenerator` 的累计和 + 边界 epsilon（`SPLIT_EPSILON` 只在这里拼写一次）；`new_record` 记声明侧（模式/行列数/子地形名/**proportions**/列映射）；`pair` 按 cfg 身份配对 `_get_terrain_mesh` ↔ `_add_sub_terrain`；`check` 列全部问题（未知子地形/配对异常/缺格重格/越界/随机模式声称列映射）。**2026-09-18 加齿**：`check` 现在把声明的列映射**回代本规则**（`column_split(proportions, num_cols)`），不一致即拒 —— 一份"来自另一条规则、但自身完全自洽"的记录此前能通过（实测旧版 `check` 返回 `[]`），这正是 PLAN #27 ② 的可判定替代（"谁是解释者"写不进 `test_component_ownership.py` 的所有权表：那张表只认对 scene 名的**写入**）。消费边界：`tools\verify\terrain_split_probe.py::record()` 调 `check`，不通过即 `SplitRecordError` |
...
| `tools\verify\terrain_preflight.py` | **开训前地形预检**（skill `isaaclab-pretrain-check`）：离线生成全部子地形 + 粗糙度统计（foot-plate relief 核心指标）+ PNG 渲染到 `_tmp_terrain_previews\` + **几何摘要**（`geometry_digest`：vertices+faces+origin，固定 dtype/字节序；生成前 `seed_rngs` 播种 numpy 与 torch 全局流，使摘要只随 version/difficulty/seed/name 变）。它是**离线预览的回归基线**，不是真跑所站地形的证据（真跑路径不播种全局 RNG，见 `versions\lizard\PLAN.md` #18 ⑤a/⑤b）；`--self-test` 进离线套件 |
...
| `tools\verify\check_split_probe_wait.py` | **P005 闸门**（套件末条，stdlib-only，~0.4s）：`terrain_split_probe.install()` 在 cfg 构造期（pre-Kit）被调用，自己不能 import `isaaclab.terrains`（P004 的毒），只能等框架 import。**等的载体不许是 `sys.meta_path` finder**——导入在第一个给出 spec 的 finder 处停止，Kit 的 finder 排在我们前面 ⇒ 钩子从不被问，`_RECORDS` 空、消费端拒绝（2026-09-18 参数网格主线真跑全灭，进程内实测 `installed=True patched=False`）。本闸门把"模块由别人放进 `sys.modules`"当输入，断言：外来导入后补丁仍落地、`install()` 自己不 import、watcher 打完即从 `builtins.__import__` 摘掉。**反证已跑**：把等待换回 finder ⇒ 本闸门报红并点名 "position-dependent again"。见 `docs\pitfalls.md` P005 |
...
| `tools\runrecord\binding.py` | **绑定原语（一处家，PLAN.md #27 ①）**：`sha256_bytes` / `sha256_file`（裸 hex，分块读 —— checkpoint 也走它）/ `git_run` / `git_rev` + `REV_LENGTH`（= 12，记录用的那一份拼写；`git --short` 的长度属 git 自己、随对象库增长而变）。stdlib-only、不 import 本仓任何东西（栈底）；`record.py` 的离线测试要求 eval 记录不引 torch/numpy/random，本模块同样干净。`provenance` 保旧名转发；`record.file_sha256` 只在这里加 `sha256:` 前缀（那是 eval 记录自己的拼写 ⇒ 迁移不动任何已落盘的值，只有 rev 由 7 变 12）。单源由 `tools\verify\check_record_bindings.py`（套件末条）看守：**四条签名**（文件摘要 / 截断 rev / 自选 rev 拼写 / 第二处 rsl_rl 身份拼写）+ 文件内自检 + 反证（指向含副本的目录 ⇒ 非零退出并点名行号）；天花板写在其 docstring（查"这一行站在哪一边"，不查语义） |
| `tools\runrecord\provenance.py` | **代码来源规则（单一实现）**：闸门与运行记录共用（内容摘要与 rev 拼写自 2026-09-18 起取自 `binding.py`，本模块只留规则）。git 树 = rev + `diff HEAD` 摘要 + porcelain 摘要 + 未跟踪文件清单（标注"需归档"）；editable 包 = 源码树，发行版 = 版本号（两者不可互换）；`rsl_rl_id()` 给出组合键用的短标识；`now()` 统一时间戳格式；路径经 `cfg_snapshot.relativize` |
