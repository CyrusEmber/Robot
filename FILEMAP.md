...
| `versions\lizard\obs_protocol_anchors.json` | **已审协议摘要与分组宽度**：`protocols[身份]` = `digest`/`dims`/`dims_digest`/`label`/`purpose`。闸门**只读不写**（自动重算摘要 = 给刚写的东西盖章）；未被任何任务引用的协议必须写明用途；空 `dims` 表示"没有真跑量过"，不是零；宽度由 `dims_digest` 自钉（原地改一个宽度会红），且必须覆盖该协议全部 live 组、值为正整数 |
