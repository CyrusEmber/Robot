...
| `tools\verify\offline_suite.py` | **离线套件调度器（唯一检查清单）**：并行跑全部检查（`--jobs` 默认 `min(6, 核数/2)`，每项独立进程/独立 temp，互不共享状态），失败即停队列——在跑的收尾、未启动的标 `skipped`、失败项回放全量输出；全绿才打 `ALL_OFFLINE_CHECKS_PASSED (n/n in ~35s, serial NNs)`。**清单只此一份**：bat 不再逐条列检查（两份清单必然漂移成"跑的不是写的那份"）。`--verbose` 打全量、`--list` 列清单、`--self-test` 自检调度器（退出码识别 + fail-fast 真跳过队列 + 坏解释器按失败报而非崩）。**契约**：每项只许读仓或只写自己 temp（并行下共享写路径从"互相覆盖"变成"竞态"）。**预算**：单条 25s / 总和 400s 两个常量在 runner 顶部，超预算先单独重跑再定罪（一波里的慢是机器负载，不是成本），出口只有"改便宜"或同一 commit 抬数。规则与新增检查的流程见 `OFFLINE_CHECKS.md` |
