...
| `recipe.py` | **配方的声明式构建**（`ARCH_PLAN.md` §2.4 阶段 B3 起步）：`RECIPES` 给出每个配方的**有序元素表** + 它声称复现的注册任务 id；`build(version, play=)` = 共享接线（`LizardRoughTeacherEnvCfg(params_version=…)`，版本作为**字段**传入 ⇒ 不经版本类）+ 该配方的元素；`pending()` 列出**尚未声明**的版本（未声明 ≠ 通过）。硬 A 由 `tools\verify\check_recipe_build.py`（套件 `[41]`）以冻结 golden 比对 |
...
| `versions\lizard\obs_protocol_anchors.json` | **已审协议摘要与分组宽度**：`protocols[身份]` = `digest`/`dims`/`label`/`purpose`。闸门**只读不写**（自动重算摘要 = 给刚写的东西盖章）；未被任何任务引用的协议必须写明用途；空 `dims` 表示"没有真跑量过"，不是零 |
...
| `tools\verify\check_obs_protocol.py` | **协议闸门**：自洽（key/摘要/内容）+ 已审锚点 + golden 侧逐任务比对 + `--live` 实构 cfg 侧逐任务比对 + 覆盖三条件（任务键集/引用存在/未引用须登记用途）；`--only` 按任务过滤 |
