...
| `convert_urdf.py` | URDF → USD（Isaac Lab 训练用资产，输出 `assets\lizard\lizard.usda`） |
| `flatten_usd.py` | 压平 URDF-importer-3.0 层级（IsaacLab issue #5126 workaround） |
| `convert_stl_to_obj.py` | STL → OBJ 转换并重写 URDF 引用 |
| `patch_kfe_axis.py` | URDF 手术：kfe 轴 Z→Y + 对称限位（旧版一次性脚本，仅考古） |
| `patch_stance.py` | URDF 手术：把自然站姿烘进零位（旧版一次性脚本，仅考古） |
| `inspect_blend.py` / `inspect_glb.py` | 无头 Blender：dump blend 骨架层级 / glb 零件包围盒 |
| `dump_all_parts.py` | dump blend 全部散件 bbox + 球形度（关节球定位用） |
...
| `view_lizard.py` | Isaac Sim GUI 里看机器人（零动作持默认位姿） |
| `debug_pose.py` | reset 后立即 dump 全部腿关节轴心世界坐标 |
| `joint_check.py` | reset 后打印关节角（验证默认位姿加载） |
| `diagnose_nan.py` | Flat 任务 NaN obs 诊断（历史问题排查用） |
| `test_staged_curriculum.py` | 课程组件离线单测（mock managers，不起仿真） |

### 训练工具与 UE 导出

| 文件 | 作用 |
|---|---|
| `dump_tb.py` | TB 事件文件 → CSV（版本记录用：`--log_dir <run目录> --out versions\vN\tb_scalars.csv`） |
| `read_curriculum.py` | 从 tfevents 读课程终值（terrain level 等） |
| `export_ue.py` | SSOT → UE 工件（关节映射/参数打包，盲部署前置） |
