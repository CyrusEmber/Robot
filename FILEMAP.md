...
| `tools\verify\check_recipe_map.py` | **配方身份闸门（ARCH_PLAN 2.1 离线，身份半边）**：`validate()` 纯函数。用 `ast` 解析 `rl_exp\tasks\__init__.py` 的 `gym.register` 调用（**不 import registry** ⇒ 不被树里半成品拖红；`[24]` 走真注册表，两者不一致本身可查）。查：任务键集 == 注册键集（缺/多即红）、映射指向的配方存在、配方键形如 `<id>@<正修订>`、`line` 必须是已发现线、配方字段白名单（夹带 per-run 即红）、**`env_cfg_entry`/`agent_entry` 与注册逐字一致**（抓静默重定向）、声明的 `legacy_task_version` 必须是任务 id 的 dash 分词（`v1` 不被 `Lizard-Rough-v14` 满足） |
| `tools\verify\test_recipe_map_gate.py` | 上者负测试（16 例 + 真注册表解析）：未登记/多出映射、悬空配方、env/agent 重定向、悬空线、键缺修订或修订为 0、缺字段、per-run 夹带、非 `v<N>` 声明、`v1` vs `v14` 子串陷阱、非 `module:qualname`、format 漂移，全部必须触发；另钉死"从真实 `tasks\__init__.py` 读出 34 个任务"（首版漏读 `id=` 关键字参数 ⇒ 空注册表让闸门静默通过） |
