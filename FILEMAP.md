...
| `tools\verify\offline_suite.py` | **离线套件调度器（唯一检查清单）**：34 项并行跑（`--jobs` 默认 `min(6, 核数/2)`，每项独立进程/独立 temp，互不共享状态），失败即停队列——在跑的收尾、未启动的标 `skipped`、失败项回放全量输出；全绿才打 `ALL_OFFLINE_CHECKS_PASSED (34/34 in ~35s)`。**清单只此一份**：bat 不再逐条列检查（两份清单必然漂移成"跑的不是写的那份"）。`--verbose` 打全量、`--list` 列清单、`--self-test` 自检调度器（退出码识别 + fail-fast 必须真跳过队列） |
| `tools\verify\run_offline_checks.bat` | **离线全套一键**（34 项：pin/parity/recovery/staged 课程/teacher 网络/student 网络/v3 课程+环形/obs 布局/v5 奖励/v5 SIR/v11 联合 SIR/版本文档/v12 噪声模型/pxr 泄漏闸/续训状态/tb 抽样/v13 跟踪核/v14 摔倒闸/验收量测（同帧+abs 侧滑）/eval 帧契约 v2/configclass 字段面 + 其负测试/配置序列化器/**配方 golden 锁 + 其负测试/运行记录/配方线发现 + 其负测试/生命周期闸门 + 其负测试/配方身份映射（含 `--bind-config`）+ 其负测试/套件横幅卫生/params 隔离**，秒级不起仿真）；**本体只剩主机 python 引导**（`paths.yaml` → `host_paths.py` → `RL_ISAAC_ROOT` 兜底）后调 `offline_suite.py` 并行跑，额外参数原样透传（`--jobs N` / `--verbose` / `--list` / `--self-test`）；改 tasks 或 harness 后、commit 前必跑 |
