...
| `tools\runrecord\rebuild.py` | **恢复演练工具（ARCH_PLAN 1.5，按需，非所有 run 的强制入口）**：`--capture <run_dir> <dest>` 按 T1 声明取材（干净代码按 rev 可取回、脏树 diff 与未跟踪代码**必须**有 `--archive` 落点，否则**拒采且不写材料**，只留拒采记录 —— `PLAN.md` #18 变成闸门）+ ckpt 与 run 记录拷贝并逐文件摘要；`--check <dest> --root <重建位置>` 查五问：scope（重建了什么 / 复用了哪些依赖；`--root` 落在原树内 ⇒ "只换配置"不算材料恢复演练）、材料摘要仍对、导入模块与资产锁落在重建位置（`--dep` 声明复用依赖）且**不落回 `--original` 给的原源码/资产树**、ckpt `infos` 回指本记录 T1、原目录可读性只作信息行（v0.16：不做系统级访问切断）；`--maintest` 删一个必需载荷必须失败、还原必须通过。不起仿真，只到"配置与加载级" |
| `tools\verify\test_rebuild_gate.py` | 上者离线验证（30 项，复用 `test_run_manifest._record` 造 run 记录）：漂移脏树拒采、无归档拒采、带归档落盘且摘要一致、未跟踪代码内容缺失拒采、材料完好/来源/载荷绑定三行、解析落回原树或未声明位置判失败、未给 `--original` 记未知、原目录可读只作信息、只换配置不算演练、删载荷必失败+还原必通过、被改载荷与冻结后被改记录均不得通过、拒采记未知（退 2）而失败记阻塞（退 1） |
