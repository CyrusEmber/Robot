...
本轮已落（零冲突部分，**未改任何 `status`、未退休任何线**）：`rl_exp/versions/lines.json`（`format`/`revision`/`lines`，键 = `recipe_lines.discover()` 的线句柄）+ 套件 `[29] check_recipe_registry.py` / `[30] test_recipe_registry_gate.py`。未执行：A3（`check_cfg_lock` 身份改消费显式映射，等锁格式 v3 结构冻结后重读）、A0（等并行批次落地）。
