...
> 落地后挂账归 `PLAN.md` #14（归档敞口另记 #18）。修订：v0.17（2026-09-15，**1.5 重定位为用户拍板口径**：日常运行仍由机器路径配置连接现有环境，不复制源码/不重建 venv/不做隔离目录；1.5 从主交付出口移出，改为**条件触发的独立恢复演练**，按次声明"哪些材料重建、哪些依赖复用"及其结论强度；`rebuild.py` 保留为演练工具，非强制入口）· v0.16（2026-09-15，**用户拍板收窄 1.5 验收强度**：不做账号/ACL 等系统级访问切断，判据改为"解析不落回原树"，出口措辞改"受检查路径上的配置与加载级重建通过"并显式承认边界）· v0.15（2026-09-15，1.5a 隔离重建闸门离线落地——取材严格拒采 + 来源断言 + 缺件负测试，`[27]`；重建根/干净解释器与归档落点仍未定）· v0.14（2026-09-15，1.4a 离线执行——B 层分支级 + c_k 迭代边界，`[15]` 26/26 + 7 例反证；1.4b C 层真跑仍待执行）· v0.13（2026-09-15，1.3a 离线落地——注册表 + v2 载荷、两条 SIR 线 + c_k 解耦、声明门与开关正名、T1 记实际恢复结果）
>
