...
> 最新修订：**v0.19（2026-09-15，详见下行修订链）**。下行保留此前实施记录。
...
- 已发布入口增加新训拒绝属于用户可见行为收紧：先发布弃用/迁移提示、后启用阻断，遵循公开接口的先弃用后变更要求。提示期、正式生效时间和后继线需显式记录；不能在添加目录时自动把旧线变为 retired。若旧入口尚未接线，只能声明新入口限制有效。
...
| L06 迁移生效 | 弃用提示期及阻断期两个配置 fixture | 提示期保留旧公开行为并给迁移说明；到已声明生效条件后才执行阻断；生命周期不进入 env/agent 快照或改变配方摘要 |
