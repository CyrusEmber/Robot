...
| 1.5 | 隔离重建（一个新 run，独立目录 + 干净环境，只复用声明依赖） | 来源断言（无解析落回原树）与缺件负测试通过，至少一条达"**受检查路径上的配置与加载级重建通过**"（不做系统级访问切断）。**闸门已落地（离线 `[27]`）**；重建根/干净解释器、归档落点与主体 run 未定 |
...
**1.5 隔离重建**（依赖 1.2 的新 run 证据与可取回归档；**2026-09-15 用户拍板收窄验收强度**）

选一个新 run，仅使用其声明的代码、安装产物、配方、checkpoint 与资产，在独立目录与干净环境里重建，只复用显式声明的运行时依赖。**换工作目录不足以通过验收**：重建进程不得继承指向原仓库的 `.pth`、editable 安装或 `PYTHONPATH`，且必须证明**实际导入的模块与解析到的资产都落在重建目录或已声明依赖位置**。不移动或删除原工作树。

- **环境边界**：不继承指向原仓库的 `.pth`、editable 安装、`PYTHONPATH` 或资产搜索路径；缓存使用独立空目录，所需预置缓存必须作为声明产物核验。允许复用预装框架、解释器、二进制运行时等运行时依赖，但须逐项记录版本/来源与验证边界；不能核验的必需依赖记未知，不计通过。
- **来源断言**：在重建进程内记录实际导入模块的真实路径与资产解析后的真实路径（解析链接），逐项判定是否落在重建目录或显式声明且已核验的依赖位置；**任一项落回原源码/资产目录 ⇒ 失败**。原目录"仍然可访问"本身**不再判失败**（见下条边界）。
- **边界声明（本次范围）**：不建专用账号、不改 ACL，**不做操作系统级访问切断**，因此不声称"系统已阻断所有原目录访问"；本步只证明**受检查路径**（import 与资产解析）上没有回退到原树，不覆盖任意隐藏读取（包内部、缓存、环境变量等旁路）。需要更强保证时另找一台机器或独立沙箱环境验证。
- **正向验收**：核验代码与资产内容摘要、完整 env/agent 配置及 T1 生效值、模型加载；配置中的运行目录/设备重定位逐项记录映射，不整树忽略。保存验证对象、环境、方法、容差和实际来源清单。
- **缺件负测试**：在可丢弃的重建副本中移除一个必需文件（如引用 mesh 或动态模型源码），重建必须明确失败，不得从原目录、共享缓存或未声明下载回退补齐；恢复该文件后重建通过。
- **出口**：正向验收、来源断言和缺件负测试均通过，方可标"**受检查路径上的配置与加载级重建通过**"（措辞限定范围，不再用"已验证重建"，以免暗示系统级隔离）。不承诺推理或训练结果等价；若某项检查条件不可验证，列出缺口并不得宣告 1.5 通过。

**1.5a 准备件（已落地 2026-09-15，离线闸门 `[27]`）**：`rl_exp/tools/runrecord/rebuild.py` 把上述四项做成命令，且**取材阶段严格拒采**（用户拍板：缺归档落点一律不写材料、评级记未知）：`--capture` 取代码（干净树按 rev 可取回；脏树 diff 与**代码根内的未跟踪文件**必须有 `--archive` 落点，否则拒采且只留拒采记录）、资产（锁文件须在 run 的 revision 上）、配方与 ckpt（逐文件摘要）；`--check` 查材料摘要、导入模块与资产锁是否落在重建根（`--dep` 为显式声明依赖）、**是否有任何解析落回 `--original` 给的原源码/资产目录**（落回即失败）、ckpt `infos` 是否回指本记录 T1；`--maintest` 删一个必需载荷必须失败、还原必须通过。全离线（不 `gym.make`），故只到"配置与加载级"。实测负证据：R1 真 run 被拒采（记录 `cc25f24b` ≠ 现树 `13aee682`，脏树 diff 内容从未落盘；未跟踪 `spider/` 无落点）。**仍未做**：重建根与干净解释器（自定义 `.pth`，不继承原仓 `.pth`）、归档落点（#18）、"取材窗口内"的新主体 run。
