...
| 1.5 | 恢复演练（**条件触发的独立验收**，不进主出口；日常运行仍走路径配置连现有环境） | 演练四步齐 + 来源断言（解析不落回原树）+ 缺件负测试通过，按次声明"重建/复用"清单与结论强度。**工具已落地（离线 `[27]`）**；不强制每次训练走它 |
...
- **来源断言**：在演练进程内记录实际导入模块与资产解析后的真实路径；**任一项落回原源码 / 资产目录 ⇒ 失败**。只换配置、材料仍在原地者按上表直接判"不构成材料恢复演练"。
