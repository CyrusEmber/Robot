...
## P010 把瞬时量当姿态、把参考当命令（同一天三错，同一个读法病）

**日期**: 2026-09-23
**影响**: 一天内三次结论都进过记录与在办事项：①"lf（左前）的目标要求脚底入地"②"符号翻转来自 `ω×r` 转动项"
③"参考越限 ⇒ 接口有缺陷 ⇒ 该裁剪"。①②已撤回，③已收窄为"待证据"。若不撤，会把"垫着走"的成因误指到
执行器/资产/接口，并向 v2 提一个**会砍掉能力**的裁剪方案（实测最坏两格拆掉 56–67% 的 `effort_limit`）。
三条读数与撤回链条见 `acceptance/records/2026-09-23-lizard2-v1-gait-skate.md` 的 ⑦/⑧/⑨。

### 症状

三处的表现各不相同，但都是"**一个分量被当成了一个量**"：

1. **`J·Δθ` 被当成目标姿态**：一阶预测 `脚底预测 = 实测净空 + J·Δθ + (ω×r)_z` 里，`J·Δθ` 是
   "**这一帧还差多少修正**"——脚当前比目标高，它就为负——**不是**"目标姿态有多低"。
   用它的（无载帧）中位数当"目标净空"，就把每个摆动段里修正方向不同的相位混在一起了。
2. **交替相位被当成左右不对称**：两条腿交替摆动，各自"无载帧集合"覆盖的相位不同，
   "lf 负 / rf 正"可以纯由相位构成。三次假设（相位、转动项、策略）都没先做**同一相位**的对照。
3. **位置参考被当成位置命令**：位置 PD 下 `τ ≈ Kp(参考−实际) − Kd·速度`，**更远的参考就是产生驱动的手段**；
   关节还没到限位时把参考裁到限位会**直接减半驱动**。而"越限帧误差更大"也是**构造性**的（参考更远，差就更大）。

### 根因

前两处是**差分/一阶量**：它们的符号和大小描述"与当前状态的差"，只有在一个相位内、且差相对动作量级较小时，
才近似读作姿态。第三处是**控制论混淆**：位置 PD 的输入是**参考**，它经增益才变成力矩；参考越界恰恰是
"要更多力矩"的正常手段，把它当"命令超范围"就丢掉了增益这一环。

### 修复（已落）

- 预测**拆项**：`diag_metrics.target_vertex_delta_parts` 分开线性项与转动项；探针同时报
  `swing_mid_*`（摆动中段帧）与 `swing_peak_*`（摆动最高帧），两组并列。
- 越限读数**分情况**：`actual_at_stop_frac`（关节是否真贴限位）、`actual_at_stop_dwell_max_frames`（贴多久）、
  `out_of_range_actual_at_stop_frac`（**只在越限帧里**贴住的比例）、`clip_removed_torque_p50_nm`
  （`Kp × |参考 − 投影回限位|`，只算关节未贴住的越限帧），`Kp`/`effort_limit` 读**实例化 cfg**。

### 通用规则

**一阶量、差分量、增益后的量，天生是"分量"，不是"量"**：`J·Δθ` 是修正量、`Kp·Δe` 是力矩、
某个速度是**某一个参考点**的速度。读它们之前先写明"这个量在命名里属于谁"，并配一条
**同相位对照**或**已知答案的自检**；两者都给不出时，那个数只能当线索，不能当结论。
