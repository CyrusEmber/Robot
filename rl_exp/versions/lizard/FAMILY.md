...
URDF 实测（估 SVL ≈2.0m；长轴 = **X**）。**朝向与命名（v8 起，2026-09-08）**：
球状头在 base **+X**（chest→neck 链末端，挂 4-DOF 云台颈），天线尾（2.37m 锥形）
在 **−X**；关节名 = 解剖学（v8 全链重命名）。**史注**：v0–v7 的 blend 命名与
解剖学整体 180° 相反（"neck*"=天线尾、"tail*"=球头、前后/左右腿名互换），
v6 因此判废倒走——发现过程、mesh 证据（0.83m 钝块/6982 顶点/11.06kg 颅骨）
与新旧名全表见 `v6\NOTES.md` 归因节 + `migrate_joint_names_v8.py` 头注；读
v8 前版本文档/复现旧 ckpt 时必须过一遍名映射。sprawled 姿态几何本身
