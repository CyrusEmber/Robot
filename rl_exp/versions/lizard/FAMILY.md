...
| baseline/v2 | 2026-09-21 | 支线第二版（未冻结未训练）：v1 的结论是**没学会**（拖颈蹭行过掉了只有速度与存活的口径，见 `acceptance/records/2026-09-20-baseline-flat-eval-protocol.md`），故本版只加两个变量——命令窗口 `1.0–3.0 m/s`（框架 10 s 重采样）与脚板关节失去动作权限（动作 26 → 22 维，保留 PD 与 `feet` 执行器组），其余逐项照 v1；配方元素列表与 v1 相同，差异全在 yaml；四件套见 `baseline/v2/` | （训练后补） |
