...
| v6 | 2026-09-07 | 资产前向轴转正（blend R_z(-90°) 头 +Y→+X + AXIS_MAP 轴同步，全版本锁同 commit 刷新）+ **v6.1 脊柱/尾动作解锁**（`joint_pos_spine` 0.0→0.25 读 yaml spine_scale；锁死态实测静垂 ±0.46 rad 欠阻尼摆动，用户拍板 2026-09-07）+ **v6.2 脊柱 PD 150/10→400/20**（重力矩 30–67 N·m 对 2m 杠杆偏软，家族比例 c=k/20，静垂减半，effort 80 不动）。yaml/reward/obs 逐字同 v5，动作 26 维布局不变。**已训判废**（2026-09-08 停训 8950/15000：**倒走**——骨命名与解剖学 180° 装反，"+Y→+X"转的是命名头=解剖学尾；探针 disp_head +1.2~+1.6m/100步 与 GUI 倒走并存定谳，归因链见 `v6\NOTES.md`） | 横行、倒走两连判：**"头"的判定必须锚定造型证据（球头/锥尾），禁止只信骨名**——装配验证全信骨名，名字错则全链绿灯 | [PLAN](v6/PLAN.md) · [NOTES](v6/NOTES.md) |
| v8 | 2026-09-08 | 资产**解剖学**转正 + 全关节重命名（v6 判废根因修复）：blend 再转 R_z(+180°)（对 pre-v6 净 +90°，**球头** −y→+x、天线尾→−x）+ 26 关节改名（rear→chest、tail→neck（球头颈）、neck1-3→tail1-3（天线尾）、腿 rl↔rf / lf↔rr 换正）+ AXIS_MAP 轴翻转 + 全版本 yaml 机械迁移（`migrate_joint_names_v8.py`）+ 注册 `Lizard-Rough-v8`。reward/obs/动作布局逐字同 v6.2。布局硬闸（neck_pitch 球头必 +X）入 `check_joints_v8.py` | （训练后补） | [PLAN](v8/PLAN.md) · [NOTES](v8/NOTES.md) |
| v7 | 2026-09-08 | ghost 断腿鲁棒性（提案，代码未实施）：截肢近似 DR（p=0.3 整腿 stiffness→0 + 质量 ×0.001，拓扑/维度契约不变）+ `damage_flags` 4 维进 actor obs（90→94，UE 游戏逻辑直填——部署端白送特权）+ v6 ckpt 微调（PITW"加地形→继续微调"配方）。limp 档/损伤分级/多腿/mid-episode 拍板不做 | （训练后补） | [PLAN](v7/PLAN.md) · [NOTES](v7/NOTES.md) |
...
URDF 实测（估 SVL ≈2.0m；长轴 = **X**——v6 -09-07；旧资产长轴曾为
Y，即 v5 首跑横行根因，判废归因见 `v6\NOTES.md`）。**勘误 2026-09-08（mesh
实测，用户发现）**：模型相对关节命名**镜像 180°**——头（0.83m 钝块、6982 顶点、
11.06kg 颅骨）在 **−X**（挂在 "tail_pitch" 链末端），尾（2.37m 锥形、宽
0.33→0.20 收窄）在 **+X**（挂在 "neck1-3" 链）；"lf/rf" 实为后肢、"rl/rr"
实为前肢。v0 起 blend 命名即反，历代"头 +X/+y"均为按名断言、无人看过 mesh。
**v8 决议（用户拍板 2026-09-08，推翻此前"v6 训练有效、受害者仅观感"框）**：
v6 判废（GUI 倒走不可接受——UE 交付朝向 + 验收第 0 条肉眼判据）；v8 转正
180°（球头→+X）+ 全关节解剖学重命名根治，此后命名=解剖学，本节勘误仅存史。
sprawled 姿态几何本身
