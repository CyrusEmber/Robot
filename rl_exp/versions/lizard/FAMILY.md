...
**身份（这版是什么）与教训只在这里**；FILEMAP.md 的版本表只给路径与冻结状态，不重复身份。
...
以 `check_joint_layout.py` 实测为准，禁止手算镜像——读法自 2026-09-21 起用该脚本的 **FK 段**
（`turn=` 定极性、`d=` 定行程），不再看站立态的 dy/dz 与 `achieved`。

### 机械链与轴向实测（2026-09-21，`Lizard-Baseline-Flat-Play-v2` 零动作，注入 +0.30 rad）

**命名顺序 ≠ 直觉**。按 URDF 原点距量出的骨长 = `haa→hfe` 0.502 m（股骨）、`hfe→kfe` 0.382 m（胫骨）、
`kfe→foot` 0.131 m ⇒ 关节顺序是 **haa=髋 → hfe=膝 → kfe=踝**，不是"hfe=髋"：

| 关节 | 机械位置 | 轴（机体系） | +值含义（头+x/左+y/上+z，右手法） | 镜像 |
|---|---|---|---|---|
| `haa` | 髋（股骨近端） | −X | 绕前后轴在横截面内摆整条腿：左腿 +=内收下压、右腿 +=外展上抬；脚走 **0.27 m**/0.3 rad | 要取反 |
| `hfe` | **膝**（股骨远端） | +Z | 绕竖直轴在膝处打转远端组件：左腿 +=后扫、右腿 +=前扫；脚原点只走 **15 mm**/0.3 rad | 要取反 |
| `kfe` | **踝**（胫骨远端） | −X | 绕前后轴屈伸小腿；脚走 0.039 m/0.3 rad | 要取反 |
| `foot` | 脚板 | +Y | 绕横向轴，两侧同轴同号 | 不取反（同上表） |

两条可直接引用的结论：

1. **`haa` 的位移 x 分量精确为 0.000** ⇒ 髋能垂直推地、能侧向撑，**推不出前进**（四腿关节的旋转全在 y-z 面内）。
2. 四腿关节里唯一带 x 分量的是 `hfe`，量级 15 mm/0.3 rad（脚原点；满量程 ≤0.05 m）⇒
   **不存在单关节大摆幅的前进行程**。**这是单关节诊断，不是能力上界**：`hfe × kfe` 组合在站姿高度下把
   掌板中心前后挪 **0.392 m**（2026-09-21 实测，见
   `acceptance/records/2026-09-21-lizard-leg-axis-kinematics.md` 的组合扫描小节）。其机制是"膝部偏航扫板"
   ——同一次 hfe 转动同时把掌板转过 137°（±1.2 rad 全程）——不是髋前伸后蹬。

读法与两条陷阱、以及被本表作废的一份手算推算（曾按"hfe 轴过髋"估 ±1.2 rad ⇒ 0.88 m 步幅）见
`docs/pitfalls.md` **P009**。
