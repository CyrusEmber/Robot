...
- 版本态：v0 存档（全量 DR 对照，无任务 id）· v1 已训（14000 iters，出分）·
  ...
  判废归因与证据见 `v6\NOTES.md`）· **v6 提案待训**（资产前向轴转正
  头 +Y→+X，reward/obs 逐字同 v5）
...
| v6 | 2026-09-07 | 资产前向轴转正（blend R_z(-90°) 头 +Y→+X + AXIS_MAP 轴同步，全版本锁同 commit 刷新）+ **v6.1 脊柱/尾动作解锁**（`joint_pos_spine` 0.0→0.25 读 yaml spine_scale；锁死态实测静垂 ±0.46 rad 欠阻尼摆动，用户拍板 2026-09-07）+ **v6.2 脊柱 PD 150/10→400/20**（重力矩 30–67 N·m 对 2m 杠杆偏软，家族比例 c=k/20，静垂减半，effort 80 不动）。yaml/reward/obs 逐字同 v5，动作 26 维布局不变。提案待训 | 已训版本资产换代 = 开 vN+1 不走 vN.M 修订（versioning §A）；烘焙改对象变换——transform_apply 过 bone-roll 有浮点漂移；锁死通道的静垂要在动作空间检查里显式测（`check_joints_v6.py`），不然策略缺补偿通道都发现不了；PD 偏软的判据 = 指令跟踪稳态误差 × 刚度反推重力矩 | [PLAN](v6/PLAN.md) · [NOTES](v6/NOTES.md) |
...
URDF 实测（估 SVL ≈2.0m，尾基在 base 后 1.26m；长轴 = **X**——v6 资产转正
2026-09-07，头 +X 对齐任务命令系；旧资产长轴曾为 Y，即 v5 首跑横行根因，判废
归因见 `v6\NOTES.md`）。sprawled 姿态几何本身
...
### 左右镜像符号约定（v6 起在案，写对称先验/对称 loss 前必读）

四条腿**同号同轴**（AXIS_MAP 按关节类型统一，不分左右），几何左右镜像
（镜像面 = x-z 平面，y→−y）。镜像下旋转是否反号取决于轴与镜像面的关系：

| 关节 | 轴（转正后） | 轴与镜像面 | 镜像动作符号 |
|---|---|---|---|
| haa | +X | 面内 | **要取反**（左 haa+ 抬腿 / 右 haa+ 压腿） |
| hfe | +Z | 面内 | **要取反** |
| kfe | +X | 面内 | **要取反** |
