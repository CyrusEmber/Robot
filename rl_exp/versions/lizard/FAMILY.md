...
| **Lizard-Rough-v10** | `LizardRoughTeacherEnvCfg_V10` | versions/lizard/v10（**已训完 15000 iter，判决半通过**，见下文件史行） | v8.1 之上单变量删除 tilt 终止（翻倒数据留 rollout 自供翻身梯度） |
| Lizard-Rough-Play-v10 | `LizardRoughTeacherEnvCfg_V10_PLAY` | versions/lizard/v10（已训完，判决半通过） | v10 回放 |
...
| **Lizard-Rough-v14** | `LizardRoughTeacherEnvCfg_V14` | versions/lizard/v14（**实施完成待训**，base = v13） | v14.3：终止项只剩**侧翻**（`\|sin(roll)\| > sin(70°)`，pitch 无关形，持续 0.5 s）；**头承重改为惩罚**（`head_load_penalty` = 头链竖向接触力 relu 求和 / 706 N，权重 -1.0，无阈值、无姿态门控）——前栽留在 rollout 里（保 v10 起身梯度），仰翻/趴地仍不掐；obs/DR/地形/课程同 v13 |
| Lizard-Rough-Play-v14 | `LizardRoughTeacherEnvCfg_V14_PLAY` | versions/lizard/v14（实施完成待训） | v14 回放 |
...
| v14 | 2026-09-14（v14.3 2026-09-15） | 加回摔倒闸（用户要求：roll 太歪即终止），**v14.3 起头承重只惩罚不收局**（用户拍板）：终止项 = `teacher_mdp.roll_over_trigger` —— `\|sin(roll)\| > sin(70°)` 的 pitch 无关形（`\|pg_b.y\| > sin(lim)·hypot(pg_b.y, pg_b.z)`；裸 `\|pg_b.y\|` 会把"既俯仰又侧滚"读小 → 漏收局），`RollOverTerm`（per-env dwell 0.5 s + reset 钩子），yaml `v14.roll_over`；头承重 = `head_load_penalty`（`RewTerm`，`SceneEntityCfg` 过滤 contact_forces 到头链 → 世界系 +z 力 relu 求和 / 706 N，权重 -1.0，**无阈值、无姿态门控、无足部卸载条件**，不做 c_k 缩放），yaml `v14.head_load`。前栽不再收局（保住前栽 rollout + 消掉"坡上持续前倾 + 头颈碰擦"的整个假阳面）、仰翻/趴地仍不掐 → 保 v10 起身梯度。闸 `check_terminations_v14.py`（侧翻判据 + 惩罚算术 + 接线 + v13 冻结）。评测用 Locomotion-Eval-v2（终止帧可见） | （训练后补） |
