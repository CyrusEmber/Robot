...
| **Lizard-Rough-v14** | `LizardRoughTeacherEnvCfg_V14` | versions/lizard/v14（**实施完成待训**，base = v13） | 摔倒闸（前栽 = 鼻朝下 >45° **且** 头链接触力 >10 N；或 \|roll\| >70°（重力方向）；各需持续 0.5 s）：只掐"头杵地/侧翻"，仰翻与趴地仍留 rollout（保 v10 起身梯度）；奖励/obs/DR 逐字同 v13 |
...
| v14 | 2026-09-14 | 加回摔倒闸（用户要求：平地两后脚翘起头杵地、roll 太歪即终止；单变量 = 终止项，base = v13）：`teacher_mdp.head_plant_or_roll_trigger` —— 前栽 = `pg_b.x > sin(45°)` **∧** 头链接触力 `> 10 N`（`head_body_names=[".*neck.*"]`，读 stock `contact_forces`），侧翻 = `|pg_b.y| > sin(70°)`（重力方向）；`HeadPlantRollTerm`（per-env dwell 0.5 s + reset 钩子），yaml `v14.fall_gate`。相对 v3 旧闸三点去风险：分轴（非总倾角）、前栽合取（鼻朝上/前倾但不触地都不收局 = v3 假阳病根；v14.1 由用户拍板收窄）、dwell（v3 瞬时）。仰翻/趴地不掐 → 保 v10 起身梯度。假阳闸预注册（终止占比近零 + 低速档 success_rate 不得停滞），判废回滚线 = v13。闸 `check_terminations_v14.py` | （训练后补） |
