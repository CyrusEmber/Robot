...
- 修订历史：v6.0（2026-09-07，资产转正重训版）→ v6.1（同日：**脊柱/尾动作
  ...
  ±0.27–0.46 rad + 欠阻尼摆动，策略无补偿通道）→ v6.2（同日：**脊柱 PD
  150/10 → 400/20**——重力矩实测 30–67 N·m 对 2m 长杠杆偏软；400/20 保
  家族比例 c=k/20，静垂减半，effort_limit 80 不动（瞬态削顶，implicit
  drive 削顶温和）；复测 `check_joints_v6.py`：neck1_pitch 跟踪 0.087→
  0.169、tail_yaw 0.30/0.30、无抖振）。
