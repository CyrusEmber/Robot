...
> **状态：解冻修改中（当前修订级 v5.1）**——v5 未训练，待改后重新冻结重打 tag。
> 修订历史：v5.0（2026-09-03 初版冻结，tag 当日撤，复现走 git `e08636b`：
> r_slip 一次范数 + 论文绝对米数脚环）→ v5.1（同日解冻修订：r_slip 改论文
> 平方 |v_f|²；脚环半径 ×5 掌宽换算 [0.4, 0.8, 1.3, 1.8, 2.4]；v3 回放
> 观察修正症状画像——不趴窝、只有脚动，"肚皮免费"从主因降为防御项）。
