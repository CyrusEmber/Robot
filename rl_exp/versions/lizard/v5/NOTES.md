...
> **状态：解冻修改中（当前修订级 v5.3）**——v5 未训练，tag v5 二次撤
> （2026-09-03，v5.3 SIR 修订），改毕重打。
...
- 相对 v4 的变更（obs 381 不变，任务 id `Lizard-Rough-v5`）:
  ...
  - **SIR 地形课程（v5.3）**: stock `terrain_levels_vel` →
    `SpawnWeightSIRTerrainCurriculum`（粒子=(类型,行)×80，带 [0.5,0.9]
    软边权重重采样 + 游走 0.8 + replay 0.05，每 10 迭代一块；成功 =
    20s 存活 + 位移 ≥ 0.5×命令距离）；地形 +flat 第 8 类型（比例 0.125
...
- 装配验证（2026-09-03，v5.3）: 单测 `test_v5_terrain_sir.py` 9/9
  （列→类型映射 / 初始 reset 跳过+origin 重指 / 成功三态判定 / 软边带 /
  带内重采样 / 流量不足保权 / 游走 clamp / replay / 块评估节流）；
