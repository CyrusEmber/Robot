...
> **状态：解冻修改中（当前修订级 v5.4）**——2026-09-03 冻结（tag v5 当日撤）→
> v5.1（r_slip 平方化 + 脚环 ×5）→ v5.2（两处权重符号修复）→ v5.3
> （SIR 地形课程 + flat 启动列）→ v5.4（冷启动修复：进度分制 + 带下线性
> 权重，tag v5 已三次撤、改毕重打）；症状画像随 v3 回放观察同步修正，见下。
...
| v5.4 | 2026-09-03 | **冷启动修复**（用户抓的洞："最开始肯定全都失败"）：成功判定终局二值 → 连续进度分 score = min(位移/(0.5×命令全程距离), 1) × 存活时间占比（存活因子抵消反梯翻滚滑水；线性位移分保留早摔部分进度）；带下权重硬 0 → **线性 ∝ p̂**（全失败期"摔得没那么惨"的行更重——零基础策略下论文硬指示会全零瘫痪课程）；累加器 `_successes`→`_scores`。tag v5 二次撤重打 |
...
| 测量概率 = Pr(Tr ∈ [0.5, 0.9]) | p̂ = 行均进度分；带内 1 / 带上软边归 0 / **带下线性 ∝ p̂** | 例外②：Tr 进度分代理；带下线性替代论文硬 0（冷启动梯度，v5.4） |
...
- **成功判定（v5.4）**：连续进度分 score = min(位移/(0.5×命令全程距离),
  1) × 存活时间占比——存活因子抵消翻滚滑水（反梯上摔一跤滚 3 m 非运动），
  线性位移分保留早摔部分进度；带作用于行均值分。**冷启动语义**（用户抓的
  洞）：零基础策略全行近零分，论文式硬指示会全零瘫痪课程，带下线性让
  "摔得没那么惨"的行（= 更易地形）更重 → flat/低行先吃流量 → 渐进入带。
...
7. **Tr 进度分代理**（v5.3→v5.4 两化）：论文 Tr 逐状态转移定义；v5.3 初版
   终局二值（存活 + 位移阈值）被用户抓出冷启动洞——零基础全失败 → 硬指示
   全零 → 课程瘫痪；v5.4 改连续进度分（位移线性 × 存活占比）+ 带下权重
   线性 ∝ p̂（论文硬 0），失败行间保留难度梯度；
...
9. **数值直译声明**（v5.3）：带 [0.5,0.9] / N_eval 10 / 游走 0.8 / replay
   ...
   0.05 / 存活因子 / 带下线性为我们的代理量纲与冷启动扩展（无论文对应）。
