...
- 附带发现（`DIAGNOSE.md`，固定本 ckpt）：零命令 10 s 全程**非四脚着地**——`rl_foot`
  ...
  均看不见（eval 无脚接触项）。
- 附带发现 2（`out/v10_run4`，3 seed × 3 档，9/9 一致）：平地直行**恒定 ~20° 蟹行**
  （横向速度 −0.15…−0.39 m/s，10 s 世界横移 2.3–6.3 m，与档速无关）；`overshoot`
  低速 +47…55%（跟踪账本仍 1.45–1.50，因 `track_lin_vel_xy_lin` 只罚慢不罚快）；
  前肢占空比 0.48–0.81 / 后肢 0.09–0.28。→ 与静止期三脚站同源：推力左右不对称。
