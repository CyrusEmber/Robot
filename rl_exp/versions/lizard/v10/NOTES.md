...
- 附带发现（`DIAGNOSE.md`，`out/v10_run5` 修正口径，固定本 ckpt）：零命令 10 s 全程
  **非四脚着地**——**左后**脚（`rl_foot`）全 50 Hz duty 2%、均值 1.02 N（≈0.14% 体重）、
  脚原点比贴地基线高 ~5 cm，即基本悬空；**左前**脚扛 **62%** 体重（右前 23%、右后 3%、
  左后 1%）；tilt 与 eval 几何判据均看不见。
- 附带发现 2（`out/v10_run5`，3 seed × 3 档，9/9 一致）：平地直行**恒定 ~19–21° 蟹行**，
  世界横移 2.4–6.2 m/10 s；拆分后**主因是相对机身侧滑**（yaw 帧横向速度 −0.16…−0.39 m/s，
  机身仅转 −0.6…−9.1°，≤0.9°/s）→ 不是转弯，是横着走；`overshoot` 低速 +48…54%
  （跟踪账本仍 1.45–1.50，因核只罚慢不罚快）；前两脚占空比 0.48–0.80 / 载荷 0.65–0.73，
  后两脚 0.11–0.25 → 与静止期三脚站同源：推力左右不对称。
- **口径修正（2026-09-11 测量校验）**：① 体重取真实质量 72.00 kg × g = 706.3 N（首版用
  0.5 s 瞬时足部接触力和 856 N，偏高 21%，载荷比例被整体压低）；② 四元数布局是 **xyzw**，
  手写 wxyz 分支使 yaw 差 177.6°、tilt 漏 roll——"180° 偏移"是用错布局，不是资产偏置；
  ③ `rl` = **左后**（首版写成右后）；④ json 逐脚时序是 5 帧抽 1，与全 50 Hz 口径要分开说。
