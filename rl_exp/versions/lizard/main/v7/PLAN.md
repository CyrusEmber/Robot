# v7 —— ghost 断腿鲁棒性（截肢近似 DR + damage flag obs + v8 checkpoint 微调）

> 状态：**归档**（2026-09-08 v7.1——提案未启动，ghost 线重基 v8 按序迁
> v9，用户拍板"按顺序开 v9"；实施见 `..\v9\PLAN.md`，本目录仅存档）。
> 修订：v7.0（2026-09-08 初稿，用户拍板"只练 ghost 和普通的，来当 v7"——
> limp 档弃训，mid-episode 断腿/损伤分级/多腿同断不做）。
> 本版变更 = DR 新组 + obs 加一项 + reward 豁免一项 + 训练源一项；方案细节
> 与验收判据继承 `../v6/PLAN.md`（及 v5 反划脚 KPI）全文，此处只记 v7 特有内容。
...
- **limp（软腿有质量拖着晃）弃用**：残肢近零质量动力学干净，塌缩风险低；
  档位存档于 v7 yaml 注释，训练期发现 ghost 表现不足再议
- **不做 mid-episode 断腿**：部署端（UE 游戏逻辑）直喂 damage flag，
  "从历史推断损伤"的问题在游戏里不存在——flag obs 使其无必要

## 变更（相对 v6）

1. **DR 新组 `v7.broken_leg`（reset 事件，代码待实施）**：每 env 每 reset
...
2. **obs：`damage_flags` 4 维 one-hot 进 actor 本体组**（90→94；全 0 = 健康）。
   ...
   `Lizard-Rough-v7` + V7 cfg 常驻注册（仅新增子类，v1–v6 类零改动——红线：
   已发布 term 语义不动）
3. **reward 豁免（V7 新 term/子类覆盖，不回改旧类）**：断腿 env 豁免断腿
...
4. **训练源：v8 checkpoint 初始化微调**（前提修正 2026-09-08：v6 判废——倒走，
   基座改指 v8，前置依赖：v8 已训）。obs 90→94 形状
