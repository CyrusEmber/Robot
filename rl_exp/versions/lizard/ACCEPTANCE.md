...
## 本文用法（索引，非验收记录；本节只路由与提示风险，不复述结论）

**性质**：全文按验收批次**追加**，日期序 = 追加序；后节可作废前节，被作废的旧句保留作留痕。
**取结论**：`grep "^## "` 跳节。某节的结论只在该节的「结论」与「边界 / 未覆盖 / 不得据本节宣称」小节内，别跨节拼。
**同一主题以最后一个提到它的节为准** —— 下表按此取；它只是便利，规则本身不随追加失效。
**未闭合项的读法**：意图侧台账 = `versions/lizard/PLAN.md` §5 挂账清单（带优先级记号与结案日），评测台侧 = `ablation_harness/HARNESS.md` 的「未做」列；**但验收侧开着的项仍只写在各节「边界 / 未覆盖」里**（已核：`社会控制`、`静默跳过`、`不可复跑`、`首回合失败` 等无对应挂账行）⇒ 问"还剩什么没做"要同时翻 PLAN §5 与全文的「边界」小节。本文只记已跑过的证据。

| 主题 | 最新节 |
|---|---|
| 续训状态层：载荷 / 注册表 / 适配器（离线） | §1.3a |
| 续训状态层：C 层集成级真跑 | §1.4b |
| 恢复演练、隔离重建（1.5） | §1.5a |
| 配方线生命周期与身份映射 | §2.1b（入口侧 2026-09-17 补） |
| 布局迁移 A0 | §2.4 |
| 组件库 B1 六片、单写者范围 | §B1 · 组件库切片 6 |
| 构建器 / 硬 A / 硬 B / `[41]` | §2.4 收尾 追加③ + §追加（2026-09-20）基线线独立化收尾 |
| 版本类体删除、golden `env_cfg_class` 看守 | §2.4 收尾 追加① |
| obs 协议契约与宽度 | §3.1e 补账 |
| eval 记录 | §3.2 |
| 地形映射 | §3.3 |
| 记录写侧（拒写 / compare / 原子写） | §评审修正 |
| 真跑收口、L03 真跑臂 | §主线真跑收口 + L03 真跑臂 |
| 基线线独立化 / EOL / 固定窗口评测 | §追加（2026-09-20）三节 |

**读本文必知（实测，截至 2026-09-20）**：

1. **历史记录里的 `[N]` 是当次运行编号，不是闸门身份**（`OFFLINE_CHECKS.md` §1：新条目追加、旧条目退役、历史记录按当时的 commit 解读，新记录同时引用脚本名）⇒ **别跨节按编号追**。套件总条目数另受 `MAX_CHECKS` 棘轮管（45 → 46 于 2026-09-20）⇒ 各节的成功行读数只在当日 commit 上成立。
2. **「已修」与「仍是缺口」并存**：`ClassVar 真缺口未修`（读取侧，§B3 · v6–v14 元素化）与 `不再有任何 ClassVar 缺口行`（台账打印，§收掉最后一条声明缺口）两节都未在同一句区分；另有旧边界被后节声明失效而原文保留 —— `硬 A 前置未满足`（§B0）被 §B1 · 组件库切片 2 判`自此失效`，`硬 B 只覆盖 env cfg`（§B2/B4）被 §B4 · 硬 B 加齿 判`自此失效` ⇒ **以作废句所在节为准**。
...
**性质**：**追加**条目。口径在上一节已定（由 `base.json` 决定：有母版走谱系、root 走 stock），本批把**其余 11 条补齐**，并把声明格式换成**按作者集合分组**（`format: 4`），baseline 一并重排到同一格式。改动面 = `[41]`（分组 schema + 三类作者判定 + 组非空 + 未声明配方打印）、`main/v2..v14` 共 11 个 `diff.json`、`baseline/v1/diff.json`。
...
main 线的 delta 有**三个写者**，不是两个：**元素集合**（同一条路径可被多个元素先后动过，实测 4 处：v3-v4 的碰撞栈、v5 的速度课程"装后删"、v11 替换 v5 的行 SIR、v13 替换 v5 的核）、**`components.X`**（按版本解析自身形态的组件，绝大多数路径都是它）、**`wiring`**（共享接线读本配方自己的文档——yaml 里的数值，既非元素也非组件，实测 v5/v6/v8/v12 各有 1–2 条）。

分组规则：**一个组 = 一个作者集合**，组里路径的作者必须与之**恰好相等**（路径放错组即红）。组件归属取"拥有该路径里某个名字、位置最浅"者，平手按字母；所有权表从 `[37]` **懒取**，不抄第二份。
...
**3.2c 拒绝路径（真跑）**：把 `model_850.pt` 写进已存在记录 `..._1150_..._ckpt1150`（同 run_id、异 ckpt）⇒ 拒写并**点名** `checkpoint.sha256`，提示 `--variant` / `--overwrite`；原记录未被改动（拒写在任何写入之前）。

**已观察到的记录状态（真跑，非构造）**：`assets=pass`（v14，冻结锁）／`assets=unknown`（dev 配方无锁）／`checkpoint=none`（零动作 run 的 `policy.kind=zero_action`）。
...
## 3.3 · 地形映射（离线 + 真跑，2026-09-17）

### 前提

真值在仓外（`E:\IsaacLab\...\terrain_generator.py:243-247`），`sub_indices` 生成后即丢；课程侧原有**两处逐字复制**。本节的证据顺序按施工件表：**3.3a 是闸口**，3.3c 通过后 3.3d/e 才动。
...
`160240a`（pre-Kit pxr 泄漏修复）把 `terrain_split_probe.install()` 的打补丁动作改成"等框架 import
生成器模块时再打"，载体是 `sys.meta_path` finder。当日起**参数网格地形的主线任务真跑全灭**：异常在
课程 term 构造时抛出 `terrain_map.SplitRecordError: no split record for this terrain: the probe was
not installed before the generator ran`，而进程**退出码 0**。本节记修复、真跑证据与 L03 两臂。

### 前提

- tree：IsaacLab `28a37cecdd43` + fork 补丁；本仓 rev 见末行"提交归属"。
...
（`install()` 之后），于是钩子一次都没被问过。离线复现（无 sim）同判：`STOLEN IMPORT LEAVES THE
PATCH UNLANDED`。详见 `docs/pitfalls.md` P005。
...
- 出口码修复只护两个调用点；with 体内其它异常、以及所有 `__main__` 之外的路径仍可能以 0 退出。
- 新闸门守的是**等待机制**，不是"Kit 不会再抢导入"这件事本身（那只能真跑观测）。
- 上述 run 都在脏树上、带 `RL_ALLOW_DIRTY_TREE`，不作可重建声明。
...
## 追加（2026-09-20）· 地形产物一致（`PLAN.md` #18 ② / ① / ⑤b 收口）

### 前提

- 交付件：`29dee47`（`terrain_geometry.py` / `suites v2` / 协议 v3 / `fork_patches\train_seed_rng.patch`）、`4051ed5`（计数语义修正）、`a7e0211`（双跑证据入账）。
- 树：双跑 A/B 为**干净树**；反证臂 C 与评测 smoke 走 `RL_ALLOW_DIRTY_TREE`（另一开发者当时正在编辑本文件，理由已写入各自记录；C 的 T0 有 `declaration.dirty_tree_override_reason` 明文）。
...
- `early_done_envs` 把末步超时数成"早收局"（`dones = truncated | terminated`，`termination_manager.py:104`）⇒ 实测 `captured=72` 却 `fall=0.000`，读起来像"72/72 全中途摔倒"；改印 `resets_in_rollout` / `early_terminations`（`4051ed5`）。
- 起伏的顶点法对"面比脚板格大"的网格给出假数：楼梯列 264 顶点跨 16 m，0.5 m 格报 0.87 m（真值 0.10 m 踢面）⇒ 加面尺寸闸（> 0.5 m 即记 `null`），天花板与升级路径（射线采样）写进 `foot_relief` docstring。

### 边界（不得据本节宣称通过）

- **归档位置改了**：证据落运行目录 `terrain/geometry.json`，不是 `PLAN.md` 原文的 `rl_exp/archive/<run_id>/terrain/`——它由 suite+seed 可再生、随记录入库即"取回得来"，`rl_exp/archive/` 仍留给"rev 取不回的内容"。**此替代需用户确认。**
- **"由 `rebuild.py` 核验"落地为**：`rebuild.py` 核**材料完整性**（`--check` 重算摘要），**地形一致由再生判定**。
