...
| 23 | **L03 真跑臂：缺课程状态 resume 必须硬拒**（C4 剩余入口侧，2026-09-17 起）：离线半边已有（`test_missing_state_hard_aborts_and_weights_only_opts_out`），缺真进程证据。**方案（自足）**：窗口内跑 v14 短训得到带状态 ckpt → 复制并剥掉 `infos[STATE_KEY]`（原件不动）→ `--resume` 指向它 ⇒ 期望**非零退出 + 硬拒**（而非 WARN 冷启动），T0/T1 的 `lifecycle.source_checkpoint` 指向该文件、`declaration_problems` 记缺状态；同一 ckpt 加 `--drop_curriculum_state` ⇒ 放行 + 记"显式降级"。**落点**：`lifecycle_entry_run.py` 新增 `--track resume`（三档判据）；离线不可替代。**反证**：换成带状态的 ckpt ⇒ 同一命令必须成功（证红来自"缺状态"而非命令写错） | 🟢 低（要真跑窗口） |
| 24 | **硬 B 覆盖面：agent（PPO）侧 + main 线差异声明**（2026-09-17 起）：**已完成的比记录多**——`baseline\v1\diff.json` 已有 `agent` 段（对着 `isaaclab_rl.rsl_rl:RslRlOnPolicyRunnerCfg` 逐叶列 allowed）且 `hard_b()` **确实检查**它 ⇒ ACCEPTANCE 里"硬 B 只覆盖 env cfg"**已过时，需更正**。**真实缺口**：全仓只有 baseline 有 `diff.json` ⇒ main 线 12 条配方**没有差异声明**（只有硬 A 的"与冻结 golden 逐字段一致"，它证"没漂"、不证"差异被声明"）。**口径二选一**：A 与 baseline 统一（声明"相对 stock runner cfg 的差异"）⇒ 复用现成机制但 12×~30 行手抄、与谱系语义不符；**B 谱系**（声明"相对 `base.json` 母版的差异"）⇒ 语义准、手抄少，但 `hard_b` 需新增"母本 = 另一条配方"分支（对比对象从 stock 换 `build(母版)`），**顺带解决 env 侧同缺口**（建议 B）。**步骤**：v14 试点（最新、差异最小）→ 逐条补齐 12 条 → `EXPECTED_DIFFS` 逐条钉数。**反证**：从某条声明删一行 ⇒ 必报"未声明改动"；某声明项指到不产出它的元素 ⇒ 必红 | 🟢 低（`[41]` 已在手） |
