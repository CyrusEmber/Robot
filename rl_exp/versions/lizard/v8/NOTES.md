...
- 修订历史：v8.0（2026-09-08，转正+重命名重训版）→ **v8.1（同日：r_slip 权重
  −0.003→−0.03，×10）**——v5 yaml 预注册的升级判据被 v6 run 触发：
  `Episode_Reward/feet_slide` 全程贴噪声底（−0.0028@8950 iters），反解得每步
  Σ|v_f|²≈0.93——低速"蠕动脚掌"步态（v3 划脚吸引子回归，GUI 实锤）的接触脚
  以 ~0.7–1 m/s 滑移，却只按 1/180 于 tracking 收入的价格付费。×10 后同一
  行为付 ~0.028/步（与 action_rate/dof_acc 同账本级别）；干净支撑脚（滑移≈0）
  不受罚——惩罚结构天然偏向真步态。**升级路径**：首个训练探针若 feet_slide
  仍平（贴 −0.03×0.93≈−0.028 的理论值视为"在说话"，低于此视为仍哑），×30
  （−0.09）为下一档。其余配方零改动（用户拍板包 C：单变量；脊柱解锁保留、
  命令窗 (0,3) 保留、foot roll 不锁）。
