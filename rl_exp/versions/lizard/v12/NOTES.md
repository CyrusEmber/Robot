...
LR 衰减（`DecayingLrPPO` 0.9999/iter）随 checkpoint 往返：实测恢复后
`lr=4.9745e-4 = 5e-4×0.9999^51`——本配方下 LR 连续性是真检查，不是空检查。
在线首次实证（四场景：全新 / 真续训 / 权重模式 / 无状态硬中断）见
`../v11/NOTES.md`「首跑实证」——同一机制，在 v11 上验的。
