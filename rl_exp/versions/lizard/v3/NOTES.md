...
- 结果回填（2026-09-03，训练 15555 iters 完成 + GUI 回放观察）:
  - TB 读数：`Metrics/success_rate` 全程 0.03→0.62 后回落稳定 0.43-0.47
    （≈站桩白嫖基线：命令 x~U(-1,2) 的 |v_cmd|<0.5 区占 50%，exp 跟踪核
    站立有残值）；`Curriculum/terrain_levels` 卡 1.27；tilt≈0 /
    time_out 99.9%（存活好）；`Episode_Reward/foot_clearance` 全程 ≤4.9e-5
    （防拖脚奖励从未有效触发）；无 NaN。
  - **行为（GUI 回放，用户观察 2026-09-03）**：不趴窝（肚皮离地，区别于
    家族旧趴窝 run 2026-08-28），**腿/脚在动但身体不前进**——原地划脚的
    foot-pad creeping 局部最优。步态动能有（脚动），推进为零。
  - 诊断结论：奖励结构不付摆腿推进的钱——r_fc 符号反（+0.003 奖励低悬
    脚）、无 r_slip（接触脚滑速免费）、exp 核低速白嫖。修复方案 = v5。
- 结论: 训练健康但策略收敛到"只有脚动"的无推进局部最优；v3 配方判废，
  转入 v5 反趴窝→反划脚奖励包。
