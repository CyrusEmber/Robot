...
| `SERIAL_BUDGET_S` | 205 | 全部检查各自秒数的**和**（不是墙钟） |

初始收紧依据：报告的 wave 178s × 1.15 ≈ 205s；这不是安静主机的标定值。
成功运行的实测总和低于预算 **超过 20%** 时，runner 打印 `BUDGET_RATCHET_WARNING`：
确认安静主机结果后收紧预算，不让合并省下的余量悄悄堆积。警告不会自动改预算；
并发负载及机器速度会影响计时，预算调整必须记录运行环境、口径与理由。
