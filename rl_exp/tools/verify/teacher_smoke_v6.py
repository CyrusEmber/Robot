...
v6 = v5 recipe on the axis-corrected asset (head +X). Same contract as
teacher_smoke_v5.py (which stays the axis-wrong asset's gate):
...
EXPECTED_GROUP_DIMS = {"proprio": 90, "extero": 208, "priv": 83}  # v6 recipe (= v5)
...
for required in ("track_lin_vel_xy_lin", "feet_slide", "belly_contact_force", "foot_clearance"):
    assert required in rm.active_terms, f"v6 reward '{required}' missing: {rm.active_terms}"
assert "track_lin_vel_xy_exp" not in rm.active_terms, "v6 must replace the exp tracking kernel"
assert "track_ang_vel_z_exp" in rm.active_terms, "yaw tracking stays (user decision)"
cm = env.unwrapped.curriculum_manager
assert "speed_curriculum" not in cm.active_terms, "v6 must drop the staged speed curriculum"
assert "terrain_levels" not in cm.active_terms, "v6 PLAY must drop the SIR terrain curriculum (no roaming)"
print("TERMS v6 rewards + no speed curriculum + no SIR in PLAY OK")
...
from rl_exp.tasks.teacher_env_cfg import LizardRoughTeacherEnvCfg_V6  # noqa: E402

train_cfg = LizardRoughTeacherEnvCfg_V6()
train_cfg.scene.num_envs = 2
train_env = gym.make("Lizard-Rough-v6", cfg=train_cfg)
