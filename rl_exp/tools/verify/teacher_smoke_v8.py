...
v8 = v6.2 recipe on the anatomy-correct asset (sphere head +X, joints renamed
chest/neck/tail1-3, legs swapped to anatomy). Same contract as
teacher_smoke_v6.py:
...
* v6.1 spine unlock rides the yaml spine_scale (0.25)
...
EXPECTED_GROUP_DIMS = {"proprio": 90, "extero": 208, "priv": 83}  # v8 recipe (= v5/v6)
...
for required in ("track_lin_vel_xy_lin", "feet_slide", "belly_contact_force", "foot_clearance"):
    assert required in rm.active_terms, f"v8 reward '{required}' missing: {rm.active_terms}"
assert "track_lin_vel_xy_exp" not in rm.active_terms, "v8 must replace the exp tracking kernel"
assert "track_ang_vel_z_exp" in rm.active_terms, "yaw tracking stays (user decision)"
cm = env.unwrapped.curriculum_manager
assert "speed_curriculum" not in cm.active_terms, "v8 must drop the staged speed curriculum"
assert "terrain_levels" not in cm.active_terms, "v8 PLAY must drop the SIR terrain curriculum (no roaming)"
print("TERMS v8 rewards + no speed curriculum + no SIR in PLAY OK")
...
assert spine_scale == 0.25, f"v8 spine action scale {spine_scale} != yaml 0.25"
...
from rl_exp.tasks.teacher_env_cfg import LizardRoughTeacherEnvCfg_V8  # noqa: E402

train_cfg = LizardRoughTeacherEnvCfg_V8()
train_cfg.scene.num_envs = 2
train_env = gym.make("Lizard-Rough-v8", cfg=train_cfg)
