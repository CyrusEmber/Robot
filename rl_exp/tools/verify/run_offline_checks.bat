...
python rl_exp\tools\verify\framework_pin_check.py || goto :fail

echo [2/4] freeze contracts (DR/wiring + robot block parity, DR lists, PLAY coverage, asset contract + locks)
python rl_exp\tools\verify\check_dr_parity.py --strict || goto :fail

echo [3/4] recovery vectorization parity
python rl_exp\tools\verify\test_recovery_parity.py || goto :fail

echo [4/4] staged curriculum offline test
python rl_exp\tools\verify\test_staged_curriculum.py || goto :fail
