...
1. **只读日志，不起仿真**：数据源是 run 目录的 tfevents + `params/agent.yaml` +
   `model_*.pt`；要跑 env 看行为属于 play/eval，不是本 skill
2. **结论必须带数字**：引用探针输出的 last/win 均值与 Δ%，禁止"看起来挺稳"式结论
3. **WARN 逐条判读**：探针给出的告警（NaN、reward 骤降、事件停更、ckpt 落后）必须逐条
   给出解释或处置建议，不允许无声跳过
4. **判读标准对照版本 NOTES**：打开该版本 `versions/<family>/<vN>/NOTES.md` 的验收线 /
   启动前警示，对照着给"继续训 / 建议查 X"结论
...
- 用户在训哪个版本 → 对应实验目录 `logs/rsl_rl/<experiment_name>/`（命名 = runner 的
  `experiment_name`）
...
```
<venv python> <REPO>\<robot>_exp\tools\trainlog\probe_run.py [--exp <版本子串>]
    [--run logs/rsl_rl/<exp>/<ts>] [--window N] [--tags Loss,Policy]
```
...
- 每行 = tag、last 值、窗口均值、对前一窗口的 Δ%（趋势方向即健康方向；error/fps 类
  大小语义相反）

### 第 3 步：判读要点

- **起步 sanity（约百 iter）**：无 NaN、reward 非零、`Episode_Termination/*` 计数正常
  （time_out 占比高 = 存活好；fall/tilt 类暴涨 = 趴窝信号）
- **课程活性**：`Curriculum/terrain_levels` 长期贴 0 或顶格不动 → 对照该版本 PLAN 的卡排
  判据；`*/stage` 是否按计划推进
- **策略塌缩**：`Policy/mean_std` 骤降到 ~0、`Loss/entropy` 暴跌 → 过早收敛信号
- **速度**：iters/h 对比该任务历史 run，掉一半先查 `Perf/collection_time` 与是否共享 GPU
- **接触栈溢出类物理问题不在本探针**：`[contact check]` 量化监控走 view_terrain
  （见 isaaclab-pretrain-check 第 3 步）
