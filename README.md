...
**fork 补丁的退出码契约**（谁启动训练，谁就得能看出它到底跑没跑）：fork 树里的门禁拒绝
一律**先打印再 `os._exit`**——`raise` / `sys.exit` 没用，sim app 收尾会在返回路径上把
退出码归一成 0，等于静默成功（2026-09-17 实测）。三个码各是各的意思，别混：

| 码 | 含义 | 例 |
|---|---|---|
| `0` | 正常结束（含零学习迭代的正常收尾） | —— |
| `2` | **有意拒绝**：门禁判否，训练从未有效启动 | 退休线拒新训／manifest 不可用（`[FATAL] … refusing to launch unrecorded`）／课程态必需但模块缺失／检查点缺声明的课程态且未 `--drop_curriculum_state` |
| `1` | **崩**：`main()` 抛未捕获异常，traceback 已在 stderr | 任何代码缺陷 |

按退出码分流的调用方（CI、`&&` 链、扫参调度）应把 `2` 记成"预期内的拒绝"、把 `1` 记成
失败上报；两者都**不代表**有训练产物。改这些码 = 改契约，须连同 `ACCEPTANCE.md` 里
按退出码记录的条目一起改（证据口径：`RC=2` + stderr 的 `[FATAL]`/`[launcher] refused` + 无新 run 目录）。
