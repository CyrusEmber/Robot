...
| 3 | **地形随机源的两处实测缺口（2026-09-18）**：① **套件里没有真正的粗糙地形列** —— `suites.py` 的 `rough_a`/`rough_b` 用单值 `noise_range=(0.05,0.05)`/`(0.15,0.15)`，`height_range` 只剩一个元素、`np.random.choice` 退化成常量，实测 p2p 恰为 0.05/0.15（均匀抬升平板），且 `suites.py:18` 的"seed 钉住 RNG 流"机制上不成立（seed 进 `TerrainGenerator` 的 **local** rng，hf 函数抽**全局** numpy 流）——套件可重复靠"范围塌缩"这条巧合；改几何 = 该开 `locomotion_eval_v3`（旧结果不得混表）。② **训练侧同一 cfg 两次真跑几何不同**：v5 的 `stepping_stones`/`random_rough` 抽未播种的全局 numpy 流、`boxes` 抽未播种的全局 torch 流（`mesh_terrains.py:348`），入口未调 `seed_everything` —— 修它属**训练随机性语义变更**。离线预览已自证可复现（`terrain_preflight --self-test`），但它只是预览回归基线，不是真跑几何证据 | 中 |
