...
| # | 事项 | 优先级 |
|---|---|---|
| 1 | **运行时验收尚未跑**（2026-09-18 起转到 **v3**：v2 已冻结封存 ⇒ ②③ 的对照若还想要，也只能在 v3 下做，且 v2 行不得混表）：① **已完成（2026-09-20，v3 零动作 smoke）**：`captured=72 == resets_in_rollout=72`、`early_terminations=0` ⇒ hook 活着（同时校正了计数口径：`dones` 含 `time_out`，末步全体超时 ⇒ 这两个数天然等于 env 总数，"early_terminations" 才是中途终止数；原先那行印 `early_done_envs=72/72` 会读成"全员中途摔倒"）；② 同一 ckpt 跨协议对照（预期 fall_rate 单调、其余指标在 ±0.02 内）；③ 基线（v13/v10 的 ckpt）在 v3 下重跑，v3 行才有对账对象；④ 拆两半：**④a 起伏已证（2026-09-20）** —— 起伏在真实生成路径逐格测量并随运行归档，离线闸按归档自述的 suite+seed 重建、逐格比（实测 `rough_a 0.030 / rough_b 0.065`，见挂账 #3）；**④b rough 两列的 completion 分布待有 ckpt 的 run** —— 零动作跑里 9 列 completion 全 ≈0.004，分布在这条跑上没有信息量，不得据此宣称"分布合理"。原 #1（路径参数化）已由 v1.5.1 落地 | 高 |
...
| 4 | **两处规格偏离待用户拍板（2026-09-20）**：① **归档位置**：地形证据落运行目录 `terrain/geometry.json`，不是 `PLAN.md` #18 原文的 `rl_exp/archive/<run_id>/terrain/`（理由：该证据由 suite+seed 可再生、随记录入库即"取回得来"，再抄一份是重复；`rl_exp/archive/` 仍留给"rev 取不回的内容"）。② **"由 `rebuild.py` 核验"的角色**：`rebuild.py` 核**材料完整性**，"地形产物一致"由**再生**判定（离线可跑）。两条都写进了 `PLAN.md` #18 与 `ACCEPTANCE.md` 的边界节，**未获确认前不得当成"照原文交付"** | 中 |
| 5 | **诊断 run 与 campaign 表之间没有闸门（2026-09-20 发现）**：`--summarize` 会把 `policy.kind=zero_action` / `smoke_only` 的行与真分数混进同一张 `summary.csv`——本轮靠"进 `--group smoke` 目录"规避，属**人工命名约定**，不是机制（`--group` 只决定目录，不校验 run 的身份）。可选做法：summarize 按 `policy.kind` 分组/标注，或闸门拒绝把 `policy.kind != "checkpoint"` 的行写进非 smoke 组。另：`results/locomotion_eval_v3/smoke/` 里 9 列 completion 全 ≈0.004，**不得**被后人当"v3 上的性能"引用 | 中 |
| 6 | **`--headless` 已弃用而本仓仍在用（2026-09-20 发现）**：IsaacLab 现为 "Omit `--viz` for default headless"，但 `run_ablation.py:83/115` 仍把它透传给 train/eval，`README.md:223`、若干 skill 与 vN NOTES 的命令行也这么写（跑起来只多一条 `DeprecationWarning`）。**未改**：删掉它是否等价，取决于任务 cfg 是否启用 visualizer（"config visualizers are enabled 时要用 `--viz none` 才强制 headless"），未确认前不动 | 低 |
