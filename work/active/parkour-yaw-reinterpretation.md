...
close_when: 用户给出裁决后按分支收：**保留现状** ⇒ 执行者把"未重跑不可比"写进该线记录/方案、并在白名单条目旁记下理由 ⇒ 关；**重解释** ⇒ 另立实施项（重跑或加注），本项在该实施项验收通过后关；**判废** ⇒ 在该线记录/方案与 `PLAN.md` 里写明判废依据（哪条读数因此不可用）+ 白名单条目保留"已知坏"标注，观测 = 该线再被引用时能看到判废状态而不是旧分数 ⇒ 关。观测收口 = 该线记录与 `baseline/v1/NOTES.md` 的口径不再互相矛盾，且白名单条目旁的"已知坏"理由仍在
---
