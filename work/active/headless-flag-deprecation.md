...
next: 先做证据分级，再决定删或留：① 查用到的 task cfg 是否声明 visualizer（**声明 ≠ 行为**，只算第一步）；② 核该声明在配置解析后是否真的启用；③ 核 `--viz none` 是否影响 `run_ablation.py` 这条调用链（透传之后有没有别的覆盖）。把三步结论按"已核到行为 / 仅读到声明"写进本项，再执行下面的关闭动作
