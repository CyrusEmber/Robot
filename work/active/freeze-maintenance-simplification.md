...
next: 第二步第三类：开训前协议绑定（`manifest.begin` 拒绝集合加一条"本版本没有冻结的验收协议或摘要不符即拒绝"）—— 正文与验收在 `eval-protocol-before-training`，本项只做编排与折入检查；反例 = 缺协议／摘要不符均拒，且先证明旧实现放行。随后第四类 `binding.git_run` 编码（反例 = 非 ASCII git 输出不再崩）。每条先证明旧实现失败，套件入口数与 `MAX_CHECKS` 不增
...
evidence: acceptance/records/2026-09-22-freeze-simplification-baseline, acceptance/records/2026-09-22-golden-subject-completeness, acceptance/records/2026-09-23-asset-lock-set-completeness
---
...
2. 补缺口、全部折入现有闸门：**golden 集合完整性已完成**（`check_golden_frozen` 主体改为从树上读，第 5 条锁已钉；该缺口是第二次发生 —— baseline 线 2026-09-17、lizard2 线 2026-09-22）；**asset lock 集合完整性已完成**（`check_dr_parity` 双向比对 + "有锁但无人读"判红，见 `evidence` 第三条；自测当场抓到 glob 少一层的实现 bug）；待做开训前协议绑定（`manifest.begin`，正文见 `eval-protocol-before-training`）、Git 输出编码稳定（`binding.git_run`）；每条先证明旧实现失败。tag 若只是辅助索引，写明边界。
