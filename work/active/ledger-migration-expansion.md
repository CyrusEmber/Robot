...
- **`next` 行预算要从 700 收回 400**：2026-09-21 实测默认读取成本在 `--list` 输出（8 项 4638 字节、
  最长行 763），所以闸门现在按项卡 `next`。落到 700 是因为另一会话的两项（485 / 629 字节）
  未提交、不归我改；**把那两项收短的那次变更里，同批把常量改回 400**（理由与实测写在常量旁）。
