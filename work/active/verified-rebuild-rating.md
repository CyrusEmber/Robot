...
close_when: 执行者在一次真 run 上走完 ①→②→③ 并观察：run 目录里未跟踪代码既有名字也有内容摘要、随仓提交后能在重建位置被核；旧记录读作 unknown 而不是被当前文件补齐 ⇒ 三件成立即关。任一未接通 ⇒ 保持 open 并写明断在哪一件。归档位置与 `rebuild.py` 角色本身的争议见 `work/active/archive-location-decision.md`，读数归 `ACCEPTANCE.md`
---
