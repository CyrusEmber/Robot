...
next: ① 冻结本线评测协议（硬前置 1）：新增 `tracking_banded_v1`/`displacement_banded_v1`/`non_foot_load_sum_v1`/`gait_swing_v1`（各带反例与自己的 judge id），并按正常/异常样本标定 `non_foot_carrier_v1` 与总量那条，接启动闸门"协议缺失或摘要不符即拒绝"；② `baseline_probe.py` 按 v1 接口改造后复测（命令 `[0.0,3.0]` 逐 env / 横向与转向恒 0 / 10 s 重采样 / 动作 30 维全覆盖 / 102 维 / 未改写下发命令）+ 零动作落地姿态目视；③ 头守卫触发未观测过（1.0 N 是类比继承）——用一条"逐渐下压"的指令测它，或在边界里一直标明；④ **待 owner 的一次判断（不是测量）**：自碰撞要不要开（筛选干净、无确认穿透，开＝物理上堵住不合理折叠，代价＝求解器接触开销；若开要重测站立/跟踪/行程）；⑤ 坏 tag `lizard2-main-v1` 留在远端但**已标记无效**（正确锚点＝`lizard2-main-v1.4` / `lizard2-main-v1.5`），是否删除由 owner 定。已完成：计数钉、obs 声明与 102 宽度实测、`diff.json` 理由、PLAN 写满、命令窗口 0–3、**执行器响应曲线**、**自碰撞筛选＋网格复核**（两条记录均已按评审修正）。
