...
next: 三件真跑核对，各自带口径：① 写侧 `substitutions` 与 `runtime.rsl_rl_id` —— `rsl_rl_id` 是 `source:<rev>`/`installed:<ver>`，而训练记录的 `code.rsl_rl.rev` 是裸 rev ⇒ 必须按 `mode` 重建同一身份再比；顺带核变体 run 的 `comparison`/`substitutions` 取值，以及 `…suite-roughb016`/`…ckpt1150` 是否落 `unknown` + pre-format reason。② `num_envs` 声明值与实际值不同的格子。③ 资产的 **fail** 路径真跑 —— **待决**：造 fail 要改冻结资产（越界），需人先定授权方式（临时副本算不算），定了才做
